/* eslint-disable prettier/prettier */
import type { AdonisEndpoint } from '@tuyau/core/types'
import type { Registry } from './schema.d.ts'
import type { ApiDefinition } from './tree.d.ts'

const placeholder: any = {}

const routes = {
  'welcome.index': {
    methods: ["GET","HEAD"],
    pattern: '/',
    tokens: [{"old":"/","type":0,"val":"/","end":""}],
    types: placeholder as Registry['welcome.index']['types'],
  },
  'home': {
    methods: ["GET","HEAD"],
    pattern: '/home',
    tokens: [{"old":"/home","type":0,"val":"home","end":""}],
    types: placeholder as Registry['home']['types'],
  },
  'about': {
    methods: ["GET","HEAD"],
    pattern: '/about',
    tokens: [{"old":"/about","type":0,"val":"about","end":""}],
    types: placeholder as Registry['about']['types'],
  },
  'welcome.landing': {
    methods: ["GET","HEAD"],
    pattern: '/welcome',
    tokens: [{"old":"/welcome","type":0,"val":"welcome","end":""}],
    types: placeholder as Registry['welcome.landing']['types'],
  },
  'welcome.about': {
    methods: ["GET","HEAD"],
    pattern: '/welcome/about',
    tokens: [{"old":"/welcome/about","type":0,"val":"welcome","end":""},{"old":"/welcome/about","type":0,"val":"about","end":""}],
    types: placeholder as Registry['welcome.about']['types'],
  },
  'welcome.features': {
    methods: ["GET","HEAD"],
    pattern: '/welcome/features',
    tokens: [{"old":"/welcome/features","type":0,"val":"welcome","end":""},{"old":"/welcome/features","type":0,"val":"features","end":""}],
    types: placeholder as Registry['welcome.features']['types'],
  },
  'welcome.contact': {
    methods: ["GET","HEAD"],
    pattern: '/welcome/contact',
    tokens: [{"old":"/welcome/contact","type":0,"val":"welcome","end":""},{"old":"/welcome/contact","type":0,"val":"contact","end":""}],
    types: placeholder as Registry['welcome.contact']['types'],
  },
  'welcome.terms': {
    methods: ["GET","HEAD"],
    pattern: '/welcome/terms',
    tokens: [{"old":"/welcome/terms","type":0,"val":"welcome","end":""},{"old":"/welcome/terms","type":0,"val":"terms","end":""}],
    types: placeholder as Registry['welcome.terms']['types'],
  },
  'schools.register.create': {
    methods: ["GET","HEAD"],
    pattern: '/register-school',
    tokens: [{"old":"/register-school","type":0,"val":"register-school","end":""}],
    types: placeholder as Registry['schools.register.create']['types'],
  },
  'schools.register': {
    methods: ["POST"],
    pattern: '/register-school',
    tokens: [{"old":"/register-school","type":0,"val":"register-school","end":""}],
    types: placeholder as Registry['schools.register']['types'],
  },
  'help.index': {
    methods: ["GET","HEAD"],
    pattern: '/help',
    tokens: [{"old":"/help","type":0,"val":"help","end":""}],
    types: placeholder as Registry['help.index']['types'],
  },
  'help.faq': {
    methods: ["GET","HEAD"],
    pattern: '/help/faq',
    tokens: [{"old":"/help/faq","type":0,"val":"help","end":""},{"old":"/help/faq","type":0,"val":"faq","end":""}],
    types: placeholder as Registry['help.faq']['types'],
  },
  'help.guides': {
    methods: ["GET","HEAD"],
    pattern: '/help/guides',
    tokens: [{"old":"/help/guides","type":0,"val":"help","end":""},{"old":"/help/guides","type":0,"val":"guides","end":""}],
    types: placeholder as Registry['help.guides']['types'],
  },
  'help.tutorial': {
    methods: ["GET","HEAD"],
    pattern: '/help/tutorial',
    tokens: [{"old":"/help/tutorial","type":0,"val":"help","end":""},{"old":"/help/tutorial","type":0,"val":"tutorial","end":""}],
    types: placeholder as Registry['help.tutorial']['types'],
  },
  'help.contact': {
    methods: ["GET","HEAD"],
    pattern: '/help/contact',
    tokens: [{"old":"/help/contact","type":0,"val":"help","end":""},{"old":"/help/contact","type":0,"val":"contact","end":""}],
    types: placeholder as Registry['help.contact']['types'],
  },
  'help.documentation': {
    methods: ["GET","HEAD"],
    pattern: '/help/documentation',
    tokens: [{"old":"/help/documentation","type":0,"val":"help","end":""},{"old":"/help/documentation","type":0,"val":"documentation","end":""}],
    types: placeholder as Registry['help.documentation']['types'],
  },
  'session.create': {
    methods: ["GET","HEAD"],
    pattern: '/login',
    tokens: [{"old":"/login","type":0,"val":"login","end":""}],
    types: placeholder as Registry['session.create']['types'],
  },
  'session.store': {
    methods: ["POST"],
    pattern: '/login',
    tokens: [{"old":"/login","type":0,"val":"login","end":""}],
    types: placeholder as Registry['session.store']['types'],
  },
  'new_account.create': {
    methods: ["GET","HEAD"],
    pattern: '/signup',
    tokens: [{"old":"/signup","type":0,"val":"signup","end":""}],
    types: placeholder as Registry['new_account.create']['types'],
  },
  'new_account.store': {
    methods: ["POST"],
    pattern: '/signup',
    tokens: [{"old":"/signup","type":0,"val":"signup","end":""}],
    types: placeholder as Registry['new_account.store']['types'],
  },
  'session.destroy': {
    methods: ["POST"],
    pattern: '/logout',
    tokens: [{"old":"/logout","type":0,"val":"logout","end":""}],
    types: placeholder as Registry['session.destroy']['types'],
  },
  'inspection.dashboard': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/dashboard',
    tokens: [{"old":"/inspection/dashboard","type":0,"val":"inspection","end":""},{"old":"/inspection/dashboard","type":0,"val":"dashboard","end":""}],
    types: placeholder as Registry['inspection.dashboard']['types'],
  },
  'inspections.schools_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/schools',
    tokens: [{"old":"/inspection/schools","type":0,"val":"inspection","end":""},{"old":"/inspection/schools","type":0,"val":"schools","end":""}],
    types: placeholder as Registry['inspections.schools_page']['types'],
  },
  'inspections.pending_schools_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/schools/pending',
    tokens: [{"old":"/inspection/schools/pending","type":0,"val":"inspection","end":""},{"old":"/inspection/schools/pending","type":0,"val":"schools","end":""},{"old":"/inspection/schools/pending","type":0,"val":"pending","end":""}],
    types: placeholder as Registry['inspections.pending_schools_page']['types'],
  },
  'inspections.school_classes_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/schools/:id/classes',
    tokens: [{"old":"/inspection/schools/:id/classes","type":0,"val":"inspection","end":""},{"old":"/inspection/schools/:id/classes","type":0,"val":"schools","end":""},{"old":"/inspection/schools/:id/classes","type":1,"val":"id","end":""},{"old":"/inspection/schools/:id/classes","type":0,"val":"classes","end":""}],
    types: placeholder as Registry['inspections.school_classes_page']['types'],
  },
  'inspections.inspect_school_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/schools/:id/inspect',
    tokens: [{"old":"/inspection/schools/:id/inspect","type":0,"val":"inspection","end":""},{"old":"/inspection/schools/:id/inspect","type":0,"val":"schools","end":""},{"old":"/inspection/schools/:id/inspect","type":1,"val":"id","end":""},{"old":"/inspection/schools/:id/inspect","type":0,"val":"inspect","end":""}],
    types: placeholder as Registry['inspections.inspect_school_page']['types'],
  },
  'inspections.store_school_inspection': {
    methods: ["POST"],
    pattern: '/inspection/schools/:id/inspect',
    tokens: [{"old":"/inspection/schools/:id/inspect","type":0,"val":"inspection","end":""},{"old":"/inspection/schools/:id/inspect","type":0,"val":"schools","end":""},{"old":"/inspection/schools/:id/inspect","type":1,"val":"id","end":""},{"old":"/inspection/schools/:id/inspect","type":0,"val":"inspect","end":""}],
    types: placeholder as Registry['inspections.store_school_inspection']['types'],
  },
  'inspections.school_details_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/schools/:id',
    tokens: [{"old":"/inspection/schools/:id","type":0,"val":"inspection","end":""},{"old":"/inspection/schools/:id","type":0,"val":"schools","end":""},{"old":"/inspection/schools/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['inspections.school_details_page']['types'],
  },
  'inspections.approve_school_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/schools/:id/approve',
    tokens: [{"old":"/inspection/schools/:id/approve","type":0,"val":"inspection","end":""},{"old":"/inspection/schools/:id/approve","type":0,"val":"schools","end":""},{"old":"/inspection/schools/:id/approve","type":1,"val":"id","end":""},{"old":"/inspection/schools/:id/approve","type":0,"val":"approve","end":""}],
    types: placeholder as Registry['inspections.approve_school_page']['types'],
  },
  'inspections.approve_and_generate_credentials': {
    methods: ["POST"],
    pattern: '/inspection/schools/:id/generate-credentials',
    tokens: [{"old":"/inspection/schools/:id/generate-credentials","type":0,"val":"inspection","end":""},{"old":"/inspection/schools/:id/generate-credentials","type":0,"val":"schools","end":""},{"old":"/inspection/schools/:id/generate-credentials","type":1,"val":"id","end":""},{"old":"/inspection/schools/:id/generate-credentials","type":0,"val":"generate-credentials","end":""}],
    types: placeholder as Registry['inspections.approve_and_generate_credentials']['types'],
  },
  'inspections.reject_school': {
    methods: ["POST"],
    pattern: '/inspection/schools/:id/reject',
    tokens: [{"old":"/inspection/schools/:id/reject","type":0,"val":"inspection","end":""},{"old":"/inspection/schools/:id/reject","type":0,"val":"schools","end":""},{"old":"/inspection/schools/:id/reject","type":1,"val":"id","end":""},{"old":"/inspection/schools/:id/reject","type":0,"val":"reject","end":""}],
    types: placeholder as Registry['inspections.reject_school']['types'],
  },
  'inspections.toggle_suspend_school': {
    methods: ["POST"],
    pattern: '/inspection/schools/:id/toggle-suspend',
    tokens: [{"old":"/inspection/schools/:id/toggle-suspend","type":0,"val":"inspection","end":""},{"old":"/inspection/schools/:id/toggle-suspend","type":0,"val":"schools","end":""},{"old":"/inspection/schools/:id/toggle-suspend","type":1,"val":"id","end":""},{"old":"/inspection/schools/:id/toggle-suspend","type":0,"val":"toggle-suspend","end":""}],
    types: placeholder as Registry['inspections.toggle_suspend_school']['types'],
  },
  'inspections.communications_global_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/communications/global',
    tokens: [{"old":"/inspection/communications/global","type":0,"val":"inspection","end":""},{"old":"/inspection/communications/global","type":0,"val":"communications","end":""},{"old":"/inspection/communications/global","type":0,"val":"global","end":""}],
    types: placeholder as Registry['inspections.communications_global_page']['types'],
  },
  'inspection.communications.global.store': {
    methods: ["POST"],
    pattern: '/inspection/communications/global',
    tokens: [{"old":"/inspection/communications/global","type":0,"val":"inspection","end":""},{"old":"/inspection/communications/global","type":0,"val":"communications","end":""},{"old":"/inspection/communications/global","type":0,"val":"global","end":""}],
    types: placeholder as Registry['inspection.communications.global.store']['types'],
  },
  'inspections.communications_school_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/communications/school',
    tokens: [{"old":"/inspection/communications/school","type":0,"val":"inspection","end":""},{"old":"/inspection/communications/school","type":0,"val":"communications","end":""},{"old":"/inspection/communications/school","type":0,"val":"school","end":""}],
    types: placeholder as Registry['inspections.communications_school_page']['types'],
  },
  'messages.send_school_communication': {
    methods: ["POST"],
    pattern: '/inspection/communications/school',
    tokens: [{"old":"/inspection/communications/school","type":0,"val":"inspection","end":""},{"old":"/inspection/communications/school","type":0,"val":"communications","end":""},{"old":"/inspection/communications/school","type":0,"val":"school","end":""}],
    types: placeholder as Registry['messages.send_school_communication']['types'],
  },
  'inspections.communications_history_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/communications/history',
    tokens: [{"old":"/inspection/communications/history","type":0,"val":"inspection","end":""},{"old":"/inspection/communications/history","type":0,"val":"communications","end":""},{"old":"/inspection/communications/history","type":0,"val":"history","end":""}],
    types: placeholder as Registry['inspections.communications_history_page']['types'],
  },
  'inspections.communication_details': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/communications/:id',
    tokens: [{"old":"/inspection/communications/:id","type":0,"val":"inspection","end":""},{"old":"/inspection/communications/:id","type":0,"val":"communications","end":""},{"old":"/inspection/communications/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['inspections.communication_details']['types'],
  },
  'inspections.reports_schools_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/reports/schools',
    tokens: [{"old":"/inspection/reports/schools","type":0,"val":"inspection","end":""},{"old":"/inspection/reports/schools","type":0,"val":"reports","end":""},{"old":"/inspection/reports/schools","type":0,"val":"schools","end":""}],
    types: placeholder as Registry['inspections.reports_schools_page']['types'],
  },
  'inspections.reports_performance_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/reports/performance',
    tokens: [{"old":"/inspection/reports/performance","type":0,"val":"inspection","end":""},{"old":"/inspection/reports/performance","type":0,"val":"reports","end":""},{"old":"/inspection/reports/performance","type":0,"val":"performance","end":""}],
    types: placeholder as Registry['inspections.reports_performance_page']['types'],
  },
  'inspections.reports_statistics_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/reports/statistics',
    tokens: [{"old":"/inspection/reports/statistics","type":0,"val":"inspection","end":""},{"old":"/inspection/reports/statistics","type":0,"val":"reports","end":""},{"old":"/inspection/reports/statistics","type":0,"val":"statistics","end":""}],
    types: placeholder as Registry['inspections.reports_statistics_page']['types'],
  },
  'inspections.reports_transfers_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/reports/transfers',
    tokens: [{"old":"/inspection/reports/transfers","type":0,"val":"inspection","end":""},{"old":"/inspection/reports/transfers","type":0,"val":"reports","end":""},{"old":"/inspection/reports/transfers","type":0,"val":"transfers","end":""}],
    types: placeholder as Registry['inspections.reports_transfers_page']['types'],
  },
  'inspections.school_report_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/reports/school/:id',
    tokens: [{"old":"/inspection/reports/school/:id","type":0,"val":"inspection","end":""},{"old":"/inspection/reports/school/:id","type":0,"val":"reports","end":""},{"old":"/inspection/reports/school/:id","type":0,"val":"school","end":""},{"old":"/inspection/reports/school/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['inspections.school_report_page']['types'],
  },
  'inspections.settings_page': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/settings',
    tokens: [{"old":"/inspection/settings","type":0,"val":"inspection","end":""},{"old":"/inspection/settings","type":0,"val":"settings","end":""}],
    types: placeholder as Registry['inspections.settings_page']['types'],
  },
  'inspection.settings.general.store': {
    methods: ["POST"],
    pattern: '/inspection/settings/general',
    tokens: [{"old":"/inspection/settings/general","type":0,"val":"inspection","end":""},{"old":"/inspection/settings/general","type":0,"val":"settings","end":""},{"old":"/inspection/settings/general","type":0,"val":"general","end":""}],
    types: placeholder as Registry['inspection.settings.general.store']['types'],
  },
  'inspection.settings.inspection.store': {
    methods: ["POST"],
    pattern: '/inspection/settings/inspection',
    tokens: [{"old":"/inspection/settings/inspection","type":0,"val":"inspection","end":""},{"old":"/inspection/settings/inspection","type":0,"val":"settings","end":""},{"old":"/inspection/settings/inspection","type":0,"val":"inspection","end":""}],
    types: placeholder as Registry['inspection.settings.inspection.store']['types'],
  },
  'inspection.settings.notifications.store': {
    methods: ["POST"],
    pattern: '/inspection/settings/notifications',
    tokens: [{"old":"/inspection/settings/notifications","type":0,"val":"inspection","end":""},{"old":"/inspection/settings/notifications","type":0,"val":"settings","end":""},{"old":"/inspection/settings/notifications","type":0,"val":"notifications","end":""}],
    types: placeholder as Registry['inspection.settings.notifications.store']['types'],
  },
  'inspection.settings.backup.store': {
    methods: ["POST"],
    pattern: '/inspection/settings/backup',
    tokens: [{"old":"/inspection/settings/backup","type":0,"val":"inspection","end":""},{"old":"/inspection/settings/backup","type":0,"val":"settings","end":""},{"old":"/inspection/settings/backup","type":0,"val":"backup","end":""}],
    types: placeholder as Registry['inspection.settings.backup.store']['types'],
  },
  'inspection.settings.security.store': {
    methods: ["POST"],
    pattern: '/inspection/settings/security',
    tokens: [{"old":"/inspection/settings/security","type":0,"val":"inspection","end":""},{"old":"/inspection/settings/security","type":0,"val":"settings","end":""},{"old":"/inspection/settings/security","type":0,"val":"security","end":""}],
    types: placeholder as Registry['inspection.settings.security.store']['types'],
  },
  'inspections.export_schools': {
    methods: ["GET","HEAD"],
    pattern: '/inspection/schools/export',
    tokens: [{"old":"/inspection/schools/export","type":0,"val":"inspection","end":""},{"old":"/inspection/schools/export","type":0,"val":"schools","end":""},{"old":"/inspection/schools/export","type":0,"val":"export","end":""}],
    types: placeholder as Registry['inspections.export_schools']['types'],
  },
  'inspections.schools_report_data': {
    methods: ["GET","HEAD"],
    pattern: '/api/inspection/reports/schools',
    tokens: [{"old":"/api/inspection/reports/schools","type":0,"val":"api","end":""},{"old":"/api/inspection/reports/schools","type":0,"val":"inspection","end":""},{"old":"/api/inspection/reports/schools","type":0,"val":"reports","end":""},{"old":"/api/inspection/reports/schools","type":0,"val":"schools","end":""}],
    types: placeholder as Registry['inspections.schools_report_data']['types'],
  },
  'inspections.performance_report_data': {
    methods: ["GET","HEAD"],
    pattern: '/api/inspection/reports/performance',
    tokens: [{"old":"/api/inspection/reports/performance","type":0,"val":"api","end":""},{"old":"/api/inspection/reports/performance","type":0,"val":"inspection","end":""},{"old":"/api/inspection/reports/performance","type":0,"val":"reports","end":""},{"old":"/api/inspection/reports/performance","type":0,"val":"performance","end":""}],
    types: placeholder as Registry['inspections.performance_report_data']['types'],
  },
  'inspections.statistics_report_data': {
    methods: ["GET","HEAD"],
    pattern: '/api/inspection/reports/statistics',
    tokens: [{"old":"/api/inspection/reports/statistics","type":0,"val":"api","end":""},{"old":"/api/inspection/reports/statistics","type":0,"val":"inspection","end":""},{"old":"/api/inspection/reports/statistics","type":0,"val":"reports","end":""},{"old":"/api/inspection/reports/statistics","type":0,"val":"statistics","end":""}],
    types: placeholder as Registry['inspections.statistics_report_data']['types'],
  },
  'inspections.transfers_report_data': {
    methods: ["GET","HEAD"],
    pattern: '/api/inspection/reports/transfers',
    tokens: [{"old":"/api/inspection/reports/transfers","type":0,"val":"api","end":""},{"old":"/api/inspection/reports/transfers","type":0,"val":"inspection","end":""},{"old":"/api/inspection/reports/transfers","type":0,"val":"reports","end":""},{"old":"/api/inspection/reports/transfers","type":0,"val":"transfers","end":""}],
    types: placeholder as Registry['inspections.transfers_report_data']['types'],
  },
  'inspections.export_transfers_report': {
    methods: ["GET","HEAD"],
    pattern: '/api/inspection/reports/transfers/export',
    tokens: [{"old":"/api/inspection/reports/transfers/export","type":0,"val":"api","end":""},{"old":"/api/inspection/reports/transfers/export","type":0,"val":"inspection","end":""},{"old":"/api/inspection/reports/transfers/export","type":0,"val":"reports","end":""},{"old":"/api/inspection/reports/transfers/export","type":0,"val":"transfers","end":""},{"old":"/api/inspection/reports/transfers/export","type":0,"val":"export","end":""}],
    types: placeholder as Registry['inspections.export_transfers_report']['types'],
  },
  'inspections.trigger_backup': {
    methods: ["POST"],
    pattern: '/api/inspection/backup',
    tokens: [{"old":"/api/inspection/backup","type":0,"val":"api","end":""},{"old":"/api/inspection/backup","type":0,"val":"inspection","end":""},{"old":"/api/inspection/backup","type":0,"val":"backup","end":""}],
    types: placeholder as Registry['inspections.trigger_backup']['types'],
  },
  'inspections.download_backup': {
    methods: ["GET","HEAD"],
    pattern: '/api/inspection/backup/download/:filename',
    tokens: [{"old":"/api/inspection/backup/download/:filename","type":0,"val":"api","end":""},{"old":"/api/inspection/backup/download/:filename","type":0,"val":"inspection","end":""},{"old":"/api/inspection/backup/download/:filename","type":0,"val":"backup","end":""},{"old":"/api/inspection/backup/download/:filename","type":0,"val":"download","end":""},{"old":"/api/inspection/backup/download/:filename","type":1,"val":"filename","end":""}],
    types: placeholder as Registry['inspections.download_backup']['types'],
  },
  'inspections.logs': {
    methods: ["GET","HEAD"],
    pattern: '/api/inspection/logs',
    tokens: [{"old":"/api/inspection/logs","type":0,"val":"api","end":""},{"old":"/api/inspection/logs","type":0,"val":"inspection","end":""},{"old":"/api/inspection/logs","type":0,"val":"logs","end":""}],
    types: placeholder as Registry['inspections.logs']['types'],
  },
  'inspections.export_logs': {
    methods: ["GET","HEAD"],
    pattern: '/api/inspection/logs/export',
    tokens: [{"old":"/api/inspection/logs/export","type":0,"val":"api","end":""},{"old":"/api/inspection/logs/export","type":0,"val":"inspection","end":""},{"old":"/api/inspection/logs/export","type":0,"val":"logs","end":""},{"old":"/api/inspection/logs/export","type":0,"val":"export","end":""}],
    types: placeholder as Registry['inspections.export_logs']['types'],
  },
  'dashboard': {
    methods: ["GET","HEAD"],
    pattern: '/dashboard',
    tokens: [{"old":"/dashboard","type":0,"val":"dashboard","end":""}],
    types: placeholder as Registry['dashboard']['types'],
  },
  'settings': {
    methods: ["GET","HEAD"],
    pattern: '/settings',
    tokens: [{"old":"/settings","type":0,"val":"settings","end":""}],
    types: placeholder as Registry['settings']['types'],
  },
  'settings.general': {
    methods: ["GET","HEAD"],
    pattern: '/settings/general',
    tokens: [{"old":"/settings/general","type":0,"val":"settings","end":""},{"old":"/settings/general","type":0,"val":"general","end":""}],
    types: placeholder as Registry['settings.general']['types'],
  },
  'settings.general.store': {
    methods: ["POST"],
    pattern: '/settings/general',
    tokens: [{"old":"/settings/general","type":0,"val":"settings","end":""},{"old":"/settings/general","type":0,"val":"general","end":""}],
    types: placeholder as Registry['settings.general.store']['types'],
  },
  'teachers.index': {
    methods: ["GET","HEAD"],
    pattern: '/teachers',
    tokens: [{"old":"/teachers","type":0,"val":"teachers","end":""}],
    types: placeholder as Registry['teachers.index']['types'],
  },
  'teachers.create': {
    methods: ["GET","HEAD"],
    pattern: '/teachers/create',
    tokens: [{"old":"/teachers/create","type":0,"val":"teachers","end":""},{"old":"/teachers/create","type":0,"val":"create","end":""}],
    types: placeholder as Registry['teachers.create']['types'],
  },
  'schools.accounts.index': {
    methods: ["GET","HEAD"],
    pattern: '/schools/accounts',
    tokens: [{"old":"/schools/accounts","type":0,"val":"schools","end":""},{"old":"/schools/accounts","type":0,"val":"accounts","end":""}],
    types: placeholder as Registry['schools.accounts.index']['types'],
  },
  'schools.accounts.create': {
    methods: ["GET","HEAD"],
    pattern: '/schools/accounts/create',
    tokens: [{"old":"/schools/accounts/create","type":0,"val":"schools","end":""},{"old":"/schools/accounts/create","type":0,"val":"accounts","end":""},{"old":"/schools/accounts/create","type":0,"val":"create","end":""}],
    types: placeholder as Registry['schools.accounts.create']['types'],
  },
  'schools.accounts.store': {
    methods: ["POST"],
    pattern: '/schools/accounts/create',
    tokens: [{"old":"/schools/accounts/create","type":0,"val":"schools","end":""},{"old":"/schools/accounts/create","type":0,"val":"accounts","end":""},{"old":"/schools/accounts/create","type":0,"val":"create","end":""}],
    types: placeholder as Registry['schools.accounts.store']['types'],
  },
  'schools.accounts.edit': {
    methods: ["GET","HEAD"],
    pattern: '/schools/accounts/:id/edit',
    tokens: [{"old":"/schools/accounts/:id/edit","type":0,"val":"schools","end":""},{"old":"/schools/accounts/:id/edit","type":0,"val":"accounts","end":""},{"old":"/schools/accounts/:id/edit","type":1,"val":"id","end":""},{"old":"/schools/accounts/:id/edit","type":0,"val":"edit","end":""}],
    types: placeholder as Registry['schools.accounts.edit']['types'],
  },
  'schools.accounts.update': {
    methods: ["POST"],
    pattern: '/schools/accounts/:id/edit',
    tokens: [{"old":"/schools/accounts/:id/edit","type":0,"val":"schools","end":""},{"old":"/schools/accounts/:id/edit","type":0,"val":"accounts","end":""},{"old":"/schools/accounts/:id/edit","type":1,"val":"id","end":""},{"old":"/schools/accounts/:id/edit","type":0,"val":"edit","end":""}],
    types: placeholder as Registry['schools.accounts.update']['types'],
  },
  'inspection.messages': {
    methods: ["GET","HEAD"],
    pattern: '/communication/messages',
    tokens: [{"old":"/communication/messages","type":0,"val":"communication","end":""},{"old":"/communication/messages","type":0,"val":"messages","end":""}],
    types: placeholder as Registry['inspection.messages']['types'],
  },
  'communication.messages.compose': {
    methods: ["GET","HEAD"],
    pattern: '/communication/messages/compose',
    tokens: [{"old":"/communication/messages/compose","type":0,"val":"communication","end":""},{"old":"/communication/messages/compose","type":0,"val":"messages","end":""},{"old":"/communication/messages/compose","type":0,"val":"compose","end":""}],
    types: placeholder as Registry['communication.messages.compose']['types'],
  },
  'communication.messages.sent': {
    methods: ["GET","HEAD"],
    pattern: '/communication/messages/sent',
    tokens: [{"old":"/communication/messages/sent","type":0,"val":"communication","end":""},{"old":"/communication/messages/sent","type":0,"val":"messages","end":""},{"old":"/communication/messages/sent","type":0,"val":"sent","end":""}],
    types: placeholder as Registry['communication.messages.sent']['types'],
  },
  'communication.messages.inbox': {
    methods: ["GET","HEAD"],
    pattern: '/communication/messages/inbox',
    tokens: [{"old":"/communication/messages/inbox","type":0,"val":"communication","end":""},{"old":"/communication/messages/inbox","type":0,"val":"messages","end":""},{"old":"/communication/messages/inbox","type":0,"val":"inbox","end":""}],
    types: placeholder as Registry['communication.messages.inbox']['types'],
  },
  'communication.messages.read': {
    methods: ["GET","HEAD"],
    pattern: '/communication/messages/read/:id',
    tokens: [{"old":"/communication/messages/read/:id","type":0,"val":"communication","end":""},{"old":"/communication/messages/read/:id","type":0,"val":"messages","end":""},{"old":"/communication/messages/read/:id","type":0,"val":"read","end":""},{"old":"/communication/messages/read/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['communication.messages.read']['types'],
  },
  'communication.messages.edit': {
    methods: ["GET","HEAD"],
    pattern: '/communication/messages/:id/edit',
    tokens: [{"old":"/communication/messages/:id/edit","type":0,"val":"communication","end":""},{"old":"/communication/messages/:id/edit","type":0,"val":"messages","end":""},{"old":"/communication/messages/:id/edit","type":1,"val":"id","end":""},{"old":"/communication/messages/:id/edit","type":0,"val":"edit","end":""}],
    types: placeholder as Registry['communication.messages.edit']['types'],
  },
  'communication.messages.update': {
    methods: ["POST"],
    pattern: '/communication/messages/:id/edit',
    tokens: [{"old":"/communication/messages/:id/edit","type":0,"val":"communication","end":""},{"old":"/communication/messages/:id/edit","type":0,"val":"messages","end":""},{"old":"/communication/messages/:id/edit","type":1,"val":"id","end":""},{"old":"/communication/messages/:id/edit","type":0,"val":"edit","end":""}],
    types: placeholder as Registry['communication.messages.update']['types'],
  },
  'communication.messages.mark_all_read': {
    methods: ["POST"],
    pattern: '/communication/messages/mark-all-read',
    tokens: [{"old":"/communication/messages/mark-all-read","type":0,"val":"communication","end":""},{"old":"/communication/messages/mark-all-read","type":0,"val":"messages","end":""},{"old":"/communication/messages/mark-all-read","type":0,"val":"mark-all-read","end":""}],
    types: placeholder as Registry['communication.messages.mark_all_read']['types'],
  },
  'communication.messages.delete': {
    methods: ["DELETE"],
    pattern: '/communication/messages/:id/delete',
    tokens: [{"old":"/communication/messages/:id/delete","type":0,"val":"communication","end":""},{"old":"/communication/messages/:id/delete","type":0,"val":"messages","end":""},{"old":"/communication/messages/:id/delete","type":1,"val":"id","end":""},{"old":"/communication/messages/:id/delete","type":0,"val":"delete","end":""}],
    types: placeholder as Registry['communication.messages.delete']['types'],
  },
  'communication.messages.send.redirect': {
    methods: ["GET","HEAD"],
    pattern: '/communication/messages/send',
    tokens: [{"old":"/communication/messages/send","type":0,"val":"communication","end":""},{"old":"/communication/messages/send","type":0,"val":"messages","end":""},{"old":"/communication/messages/send","type":0,"val":"send","end":""}],
    types: placeholder as Registry['communication.messages.send.redirect']['types'],
  },
  'communication.messages.send': {
    methods: ["POST"],
    pattern: '/communication/messages/send',
    tokens: [{"old":"/communication/messages/send","type":0,"val":"communication","end":""},{"old":"/communication/messages/send","type":0,"val":"messages","end":""},{"old":"/communication/messages/send","type":0,"val":"send","end":""}],
    types: placeholder as Registry['communication.messages.send']['types'],
  },
  'communication.notifications.index': {
    methods: ["GET","HEAD"],
    pattern: '/communication/notifications',
    tokens: [{"old":"/communication/notifications","type":0,"val":"communication","end":""},{"old":"/communication/notifications","type":0,"val":"notifications","end":""}],
    types: placeholder as Registry['communication.notifications.index']['types'],
  },
  'api.notifications.index': {
    methods: ["GET","HEAD"],
    pattern: '/api/notifications',
    tokens: [{"old":"/api/notifications","type":0,"val":"api","end":""},{"old":"/api/notifications","type":0,"val":"notifications","end":""}],
    types: placeholder as Registry['api.notifications.index']['types'],
  },
  'api.notifications.read': {
    methods: ["POST"],
    pattern: '/api/notifications/:id/read',
    tokens: [{"old":"/api/notifications/:id/read","type":0,"val":"api","end":""},{"old":"/api/notifications/:id/read","type":0,"val":"notifications","end":""},{"old":"/api/notifications/:id/read","type":1,"val":"id","end":""},{"old":"/api/notifications/:id/read","type":0,"val":"read","end":""}],
    types: placeholder as Registry['api.notifications.read']['types'],
  },
  'api.notifications.read.put': {
    methods: ["PUT"],
    pattern: '/api/notifications/:id/read',
    tokens: [{"old":"/api/notifications/:id/read","type":0,"val":"api","end":""},{"old":"/api/notifications/:id/read","type":0,"val":"notifications","end":""},{"old":"/api/notifications/:id/read","type":1,"val":"id","end":""},{"old":"/api/notifications/:id/read","type":0,"val":"read","end":""}],
    types: placeholder as Registry['api.notifications.read.put']['types'],
  },
  'api.notifications.mark_all_read': {
    methods: ["POST"],
    pattern: '/api/notifications/mark-all-read',
    tokens: [{"old":"/api/notifications/mark-all-read","type":0,"val":"api","end":""},{"old":"/api/notifications/mark-all-read","type":0,"val":"notifications","end":""},{"old":"/api/notifications/mark-all-read","type":0,"val":"mark-all-read","end":""}],
    types: placeholder as Registry['api.notifications.mark_all_read']['types'],
  },
  'api.notifications.read_all': {
    methods: ["PUT"],
    pattern: '/api/notifications/read-all',
    tokens: [{"old":"/api/notifications/read-all","type":0,"val":"api","end":""},{"old":"/api/notifications/read-all","type":0,"val":"notifications","end":""},{"old":"/api/notifications/read-all","type":0,"val":"read-all","end":""}],
    types: placeholder as Registry['api.notifications.read_all']['types'],
  },
  'api.notifications.delete_all': {
    methods: ["DELETE"],
    pattern: '/api/notifications/delete-all',
    tokens: [{"old":"/api/notifications/delete-all","type":0,"val":"api","end":""},{"old":"/api/notifications/delete-all","type":0,"val":"notifications","end":""},{"old":"/api/notifications/delete-all","type":0,"val":"delete-all","end":""}],
    types: placeholder as Registry['api.notifications.delete_all']['types'],
  },
  'api.teacher.attendance.classes.students': {
    methods: ["GET","HEAD"],
    pattern: '/api/teacher/classes/:id/students',
    tokens: [{"old":"/api/teacher/classes/:id/students","type":0,"val":"api","end":""},{"old":"/api/teacher/classes/:id/students","type":0,"val":"teacher","end":""},{"old":"/api/teacher/classes/:id/students","type":0,"val":"classes","end":""},{"old":"/api/teacher/classes/:id/students","type":1,"val":"id","end":""},{"old":"/api/teacher/classes/:id/students","type":0,"val":"students","end":""}],
    types: placeholder as Registry['api.teacher.attendance.classes.students']['types'],
  },
  'api.teacher.attendance.class': {
    methods: ["GET","HEAD"],
    pattern: '/api/teacher/attendance/class/:id',
    tokens: [{"old":"/api/teacher/attendance/class/:id","type":0,"val":"api","end":""},{"old":"/api/teacher/attendance/class/:id","type":0,"val":"teacher","end":""},{"old":"/api/teacher/attendance/class/:id","type":0,"val":"attendance","end":""},{"old":"/api/teacher/attendance/class/:id","type":0,"val":"class","end":""},{"old":"/api/teacher/attendance/class/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['api.teacher.attendance.class']['types'],
  },
  'api.teacher.attendance.store': {
    methods: ["POST"],
    pattern: '/api/teacher/attendance',
    tokens: [{"old":"/api/teacher/attendance","type":0,"val":"api","end":""},{"old":"/api/teacher/attendance","type":0,"val":"teacher","end":""},{"old":"/api/teacher/attendance","type":0,"val":"attendance","end":""}],
    types: placeholder as Registry['api.teacher.attendance.store']['types'],
  },
  'inspection.users.stats': {
    methods: ["GET","HEAD"],
    pattern: '/api/users/stats',
    tokens: [{"old":"/api/users/stats","type":0,"val":"api","end":""},{"old":"/api/users/stats","type":0,"val":"users","end":""},{"old":"/api/users/stats","type":0,"val":"stats","end":""}],
    types: placeholder as Registry['inspection.users.stats']['types'],
  },
  'inspection.schools.communication.info': {
    methods: ["GET","HEAD"],
    pattern: '/api/schools/:id/info',
    tokens: [{"old":"/api/schools/:id/info","type":0,"val":"api","end":""},{"old":"/api/schools/:id/info","type":0,"val":"schools","end":""},{"old":"/api/schools/:id/info","type":1,"val":"id","end":""},{"old":"/api/schools/:id/info","type":0,"val":"info","end":""}],
    types: placeholder as Registry['inspection.schools.communication.info']['types'],
  },
  'inspection.schools.communication.history': {
    methods: ["GET","HEAD"],
    pattern: '/api/schools/:id/communications',
    tokens: [{"old":"/api/schools/:id/communications","type":0,"val":"api","end":""},{"old":"/api/schools/:id/communications","type":0,"val":"schools","end":""},{"old":"/api/schools/:id/communications","type":1,"val":"id","end":""},{"old":"/api/schools/:id/communications","type":0,"val":"communications","end":""}],
    types: placeholder as Registry['inspection.schools.communication.history']['types'],
  },
  'inspection.communications.details.json': {
    methods: ["GET","HEAD"],
    pattern: '/api/communications/:id',
    tokens: [{"old":"/api/communications/:id","type":0,"val":"api","end":""},{"old":"/api/communications/:id","type":0,"val":"communications","end":""},{"old":"/api/communications/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['inspection.communications.details.json']['types'],
  },
  'profile': {
    methods: ["GET","HEAD"],
    pattern: '/profile',
    tokens: [{"old":"/profile","type":0,"val":"profile","end":""}],
    types: placeholder as Registry['profile']['types'],
  },
  'profile.edit': {
    methods: ["GET","HEAD"],
    pattern: '/profile/edit',
    tokens: [{"old":"/profile/edit","type":0,"val":"profile","end":""},{"old":"/profile/edit","type":0,"val":"edit","end":""}],
    types: placeholder as Registry['profile.edit']['types'],
  },
  'profile.security': {
    methods: ["GET","HEAD"],
    pattern: '/profile/security',
    tokens: [{"old":"/profile/security","type":0,"val":"profile","end":""},{"old":"/profile/security","type":0,"val":"security","end":""}],
    types: placeholder as Registry['profile.security']['types'],
  },
  'profile.preferences': {
    methods: ["GET","HEAD"],
    pattern: '/profile/preferences',
    tokens: [{"old":"/profile/preferences","type":0,"val":"profile","end":""},{"old":"/profile/preferences","type":0,"val":"preferences","end":""}],
    types: placeholder as Registry['profile.preferences']['types'],
  },
  'profile.preferences.save': {
    methods: ["POST"],
    pattern: '/profile/preferences',
    tokens: [{"old":"/profile/preferences","type":0,"val":"profile","end":""},{"old":"/profile/preferences","type":0,"val":"preferences","end":""}],
    types: placeholder as Registry['profile.preferences.save']['types'],
  },
  'profile.activity': {
    methods: ["GET","HEAD"],
    pattern: '/profile/activity',
    tokens: [{"old":"/profile/activity","type":0,"val":"profile","end":""},{"old":"/profile/activity","type":0,"val":"activity","end":""}],
    types: placeholder as Registry['profile.activity']['types'],
  },
  'profile.avatar.update': {
    methods: ["POST"],
    pattern: '/api/profile/avatar',
    tokens: [{"old":"/api/profile/avatar","type":0,"val":"api","end":""},{"old":"/api/profile/avatar","type":0,"val":"profile","end":""},{"old":"/api/profile/avatar","type":0,"val":"avatar","end":""}],
    types: placeholder as Registry['profile.avatar.update']['types'],
  },
  'auth.save_language': {
    methods: ["POST"],
    pattern: '/api/settings/language',
    tokens: [{"old":"/api/settings/language","type":0,"val":"api","end":""},{"old":"/api/settings/language","type":0,"val":"settings","end":""},{"old":"/api/settings/language","type":0,"val":"language","end":""}],
    types: placeholder as Registry['auth.save_language']['types'],
  },
  'auth.save_regional': {
    methods: ["POST"],
    pattern: '/api/settings/regional',
    tokens: [{"old":"/api/settings/regional","type":0,"val":"api","end":""},{"old":"/api/settings/regional","type":0,"val":"settings","end":""},{"old":"/api/settings/regional","type":0,"val":"regional","end":""}],
    types: placeholder as Registry['auth.save_regional']['types'],
  },
  'academic.classes.index': {
    methods: ["GET","HEAD"],
    pattern: '/academic/classes',
    tokens: [{"old":"/academic/classes","type":0,"val":"academic","end":""},{"old":"/academic/classes","type":0,"val":"classes","end":""}],
    types: placeholder as Registry['academic.classes.index']['types'],
  },
  'academic.classes.create': {
    methods: ["GET","HEAD"],
    pattern: '/academic/classes/create',
    tokens: [{"old":"/academic/classes/create","type":0,"val":"academic","end":""},{"old":"/academic/classes/create","type":0,"val":"classes","end":""},{"old":"/academic/classes/create","type":0,"val":"create","end":""}],
    types: placeholder as Registry['academic.classes.create']['types'],
  },
  'academic.classes.seed_rdc_das': {
    methods: ["POST"],
    pattern: '/academic/classes/seed-rdc-das',
    tokens: [{"old":"/academic/classes/seed-rdc-das","type":0,"val":"academic","end":""},{"old":"/academic/classes/seed-rdc-das","type":0,"val":"classes","end":""},{"old":"/academic/classes/seed-rdc-das","type":0,"val":"seed-rdc-das","end":""}],
    types: placeholder as Registry['academic.classes.seed_rdc_das']['types'],
  },
  'academic.classes.store': {
    methods: ["POST"],
    pattern: '/academic/classes',
    tokens: [{"old":"/academic/classes","type":0,"val":"academic","end":""},{"old":"/academic/classes","type":0,"val":"classes","end":""}],
    types: placeholder as Registry['academic.classes.store']['types'],
  },
  'academic.classes.show': {
    methods: ["GET","HEAD"],
    pattern: '/academic/classes/:id',
    tokens: [{"old":"/academic/classes/:id","type":0,"val":"academic","end":""},{"old":"/academic/classes/:id","type":0,"val":"classes","end":""},{"old":"/academic/classes/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['academic.classes.show']['types'],
  },
  'academic.classes.edit': {
    methods: ["GET","HEAD"],
    pattern: '/academic/classes/:id/edit',
    tokens: [{"old":"/academic/classes/:id/edit","type":0,"val":"academic","end":""},{"old":"/academic/classes/:id/edit","type":0,"val":"classes","end":""},{"old":"/academic/classes/:id/edit","type":1,"val":"id","end":""},{"old":"/academic/classes/:id/edit","type":0,"val":"edit","end":""}],
    types: placeholder as Registry['academic.classes.edit']['types'],
  },
  'academic.classes.update.post': {
    methods: ["POST"],
    pattern: '/academic/classes/:id',
    tokens: [{"old":"/academic/classes/:id","type":0,"val":"academic","end":""},{"old":"/academic/classes/:id","type":0,"val":"classes","end":""},{"old":"/academic/classes/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['academic.classes.update.post']['types'],
  },
  'academic.classes.update': {
    methods: ["PUT"],
    pattern: '/academic/classes/:id',
    tokens: [{"old":"/academic/classes/:id","type":0,"val":"academic","end":""},{"old":"/academic/classes/:id","type":0,"val":"classes","end":""},{"old":"/academic/classes/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['academic.classes.update']['types'],
  },
  'academic.classes.destroy': {
    methods: ["DELETE"],
    pattern: '/academic/classes/:id',
    tokens: [{"old":"/academic/classes/:id","type":0,"val":"academic","end":""},{"old":"/academic/classes/:id","type":0,"val":"classes","end":""},{"old":"/academic/classes/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['academic.classes.destroy']['types'],
  },
  'academic.grades.index': {
    methods: ["GET","HEAD"],
    pattern: '/academic/grades',
    tokens: [{"old":"/academic/grades","type":0,"val":"academic","end":""},{"old":"/academic/grades","type":0,"val":"grades","end":""}],
    types: placeholder as Registry['academic.grades.index']['types'],
  },
  'academic.grades.add': {
    methods: ["GET","HEAD"],
    pattern: '/academic/grades/add',
    tokens: [{"old":"/academic/grades/add","type":0,"val":"academic","end":""},{"old":"/academic/grades/add","type":0,"val":"grades","end":""},{"old":"/academic/grades/add","type":0,"val":"add","end":""}],
    types: placeholder as Registry['academic.grades.add']['types'],
  },
  'academic.grades.store': {
    methods: ["POST"],
    pattern: '/academic/grades',
    tokens: [{"old":"/academic/grades","type":0,"val":"academic","end":""},{"old":"/academic/grades","type":0,"val":"grades","end":""}],
    types: placeholder as Registry['academic.grades.store']['types'],
  },
  'academic.grades.publish': {
    methods: ["POST"],
    pattern: '/academic/grades/publish',
    tokens: [{"old":"/academic/grades/publish","type":0,"val":"academic","end":""},{"old":"/academic/grades/publish","type":0,"val":"grades","end":""},{"old":"/academic/grades/publish","type":0,"val":"publish","end":""}],
    types: placeholder as Registry['academic.grades.publish']['types'],
  },
  'academic.timetable.create': {
    methods: ["GET","HEAD"],
    pattern: '/academic/timetable/create',
    tokens: [{"old":"/academic/timetable/create","type":0,"val":"academic","end":""},{"old":"/academic/timetable/create","type":0,"val":"timetable","end":""},{"old":"/academic/timetable/create","type":0,"val":"create","end":""}],
    types: placeholder as Registry['academic.timetable.create']['types'],
  },
  'academic.timetable.class': {
    methods: ["GET","HEAD"],
    pattern: '/academic/timetable/class/:classId',
    tokens: [{"old":"/academic/timetable/class/:classId","type":0,"val":"academic","end":""},{"old":"/academic/timetable/class/:classId","type":0,"val":"timetable","end":""},{"old":"/academic/timetable/class/:classId","type":0,"val":"class","end":""},{"old":"/academic/timetable/class/:classId","type":1,"val":"classId","end":""}],
    types: placeholder as Registry['academic.timetable.class']['types'],
  },
  'academic.grades.class': {
    methods: ["GET","HEAD"],
    pattern: '/academic/grades/class/:classId',
    tokens: [{"old":"/academic/grades/class/:classId","type":0,"val":"academic","end":""},{"old":"/academic/grades/class/:classId","type":0,"val":"grades","end":""},{"old":"/academic/grades/class/:classId","type":0,"val":"class","end":""},{"old":"/academic/grades/class/:classId","type":1,"val":"classId","end":""}],
    types: placeholder as Registry['academic.grades.class']['types'],
  },
  'academic.classes.students': {
    methods: ["GET","HEAD"],
    pattern: '/academic/classes/:id/students',
    tokens: [{"old":"/academic/classes/:id/students","type":0,"val":"academic","end":""},{"old":"/academic/classes/:id/students","type":0,"val":"classes","end":""},{"old":"/academic/classes/:id/students","type":1,"val":"id","end":""},{"old":"/academic/classes/:id/students","type":0,"val":"students","end":""}],
    types: placeholder as Registry['academic.classes.students']['types'],
  },
  'academic.classes.subjects': {
    methods: ["GET","HEAD"],
    pattern: '/academic/classes/:classId/subjects',
    tokens: [{"old":"/academic/classes/:classId/subjects","type":0,"val":"academic","end":""},{"old":"/academic/classes/:classId/subjects","type":0,"val":"classes","end":""},{"old":"/academic/classes/:classId/subjects","type":1,"val":"classId","end":""},{"old":"/academic/classes/:classId/subjects","type":0,"val":"subjects","end":""}],
    types: placeholder as Registry['academic.classes.subjects']['types'],
  },
  'legacy.api.classes.students': {
    methods: ["GET","HEAD"],
    pattern: '/api/classes/:id/students',
    tokens: [{"old":"/api/classes/:id/students","type":0,"val":"api","end":""},{"old":"/api/classes/:id/students","type":0,"val":"classes","end":""},{"old":"/api/classes/:id/students","type":1,"val":"id","end":""},{"old":"/api/classes/:id/students","type":0,"val":"students","end":""}],
    types: placeholder as Registry['legacy.api.classes.students']['types'],
  },
  'legacy.api.classes.subjects': {
    methods: ["GET","HEAD"],
    pattern: '/api/classes/:classId/subjects',
    tokens: [{"old":"/api/classes/:classId/subjects","type":0,"val":"api","end":""},{"old":"/api/classes/:classId/subjects","type":0,"val":"classes","end":""},{"old":"/api/classes/:classId/subjects","type":1,"val":"classId","end":""},{"old":"/api/classes/:classId/subjects","type":0,"val":"subjects","end":""}],
    types: placeholder as Registry['legacy.api.classes.subjects']['types'],
  },
  'legacy.api.grades.class': {
    methods: ["GET","HEAD"],
    pattern: '/api/grades/class/:classId',
    tokens: [{"old":"/api/grades/class/:classId","type":0,"val":"api","end":""},{"old":"/api/grades/class/:classId","type":0,"val":"grades","end":""},{"old":"/api/grades/class/:classId","type":0,"val":"class","end":""},{"old":"/api/grades/class/:classId","type":1,"val":"classId","end":""}],
    types: placeholder as Registry['legacy.api.grades.class']['types'],
  },
  'legacy.api.timetable.class': {
    methods: ["GET","HEAD"],
    pattern: '/api/timetable/class/:classId',
    tokens: [{"old":"/api/timetable/class/:classId","type":0,"val":"api","end":""},{"old":"/api/timetable/class/:classId","type":0,"val":"timetable","end":""},{"old":"/api/timetable/class/:classId","type":0,"val":"class","end":""},{"old":"/api/timetable/class/:classId","type":1,"val":"classId","end":""}],
    types: placeholder as Registry['legacy.api.timetable.class']['types'],
  },
  'legacy.api.timetable.create': {
    methods: ["POST"],
    pattern: '/api/timetable/create',
    tokens: [{"old":"/api/timetable/create","type":0,"val":"api","end":""},{"old":"/api/timetable/create","type":0,"val":"timetable","end":""},{"old":"/api/timetable/create","type":0,"val":"create","end":""}],
    types: placeholder as Registry['legacy.api.timetable.create']['types'],
  },
  'students.index': {
    methods: ["GET","HEAD"],
    pattern: '/students',
    tokens: [{"old":"/students","type":0,"val":"students","end":""}],
    types: placeholder as Registry['students.index']['types'],
  },
  'students.create': {
    methods: ["GET","HEAD"],
    pattern: '/students/create',
    tokens: [{"old":"/students/create","type":0,"val":"students","end":""},{"old":"/students/create","type":0,"val":"create","end":""}],
    types: placeholder as Registry['students.create']['types'],
  },
  'students.store': {
    methods: ["POST"],
    pattern: '/students/create',
    tokens: [{"old":"/students/create","type":0,"val":"students","end":""},{"old":"/students/create","type":0,"val":"create","end":""}],
    types: placeholder as Registry['students.store']['types'],
  },
  'schools.classes.index': {
    methods: ["GET","HEAD"],
    pattern: '/schools/classes',
    tokens: [{"old":"/schools/classes","type":0,"val":"schools","end":""},{"old":"/schools/classes","type":0,"val":"classes","end":""}],
    types: placeholder as Registry['schools.classes.index']['types'],
  },
  'schools.classes.create': {
    methods: ["GET","HEAD"],
    pattern: '/schools/classes/create',
    tokens: [{"old":"/schools/classes/create","type":0,"val":"schools","end":""},{"old":"/schools/classes/create","type":0,"val":"classes","end":""},{"old":"/schools/classes/create","type":0,"val":"create","end":""}],
    types: placeholder as Registry['schools.classes.create']['types'],
  },
  'schools.classes.seed_rdc_das': {
    methods: ["POST"],
    pattern: '/schools/classes/seed-rdc-das',
    tokens: [{"old":"/schools/classes/seed-rdc-das","type":0,"val":"schools","end":""},{"old":"/schools/classes/seed-rdc-das","type":0,"val":"classes","end":""},{"old":"/schools/classes/seed-rdc-das","type":0,"val":"seed-rdc-das","end":""}],
    types: placeholder as Registry['schools.classes.seed_rdc_das']['types'],
  },
  'schools.classes.store': {
    methods: ["POST"],
    pattern: '/schools/classes',
    tokens: [{"old":"/schools/classes","type":0,"val":"schools","end":""},{"old":"/schools/classes","type":0,"val":"classes","end":""}],
    types: placeholder as Registry['schools.classes.store']['types'],
  },
  'schools.classes.students': {
    methods: ["GET","HEAD"],
    pattern: '/schools/classes/:id/students',
    tokens: [{"old":"/schools/classes/:id/students","type":0,"val":"schools","end":""},{"old":"/schools/classes/:id/students","type":0,"val":"classes","end":""},{"old":"/schools/classes/:id/students","type":1,"val":"id","end":""},{"old":"/schools/classes/:id/students","type":0,"val":"students","end":""}],
    types: placeholder as Registry['schools.classes.students']['types'],
  },
  'schools.classes.edit': {
    methods: ["GET","HEAD"],
    pattern: '/schools/classes/:id/edit',
    tokens: [{"old":"/schools/classes/:id/edit","type":0,"val":"schools","end":""},{"old":"/schools/classes/:id/edit","type":0,"val":"classes","end":""},{"old":"/schools/classes/:id/edit","type":1,"val":"id","end":""},{"old":"/schools/classes/:id/edit","type":0,"val":"edit","end":""}],
    types: placeholder as Registry['schools.classes.edit']['types'],
  },
  'schools.classes.show': {
    methods: ["GET","HEAD"],
    pattern: '/schools/classes/:id',
    tokens: [{"old":"/schools/classes/:id","type":0,"val":"schools","end":""},{"old":"/schools/classes/:id","type":0,"val":"classes","end":""},{"old":"/schools/classes/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['schools.classes.show']['types'],
  },
  'schools.classes.update.post': {
    methods: ["POST"],
    pattern: '/schools/classes/:id',
    tokens: [{"old":"/schools/classes/:id","type":0,"val":"schools","end":""},{"old":"/schools/classes/:id","type":0,"val":"classes","end":""},{"old":"/schools/classes/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['schools.classes.update.post']['types'],
  },
  'schools.classes.update': {
    methods: ["PUT"],
    pattern: '/schools/classes/:id',
    tokens: [{"old":"/schools/classes/:id","type":0,"val":"schools","end":""},{"old":"/schools/classes/:id","type":0,"val":"classes","end":""},{"old":"/schools/classes/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['schools.classes.update']['types'],
  },
  'schools.classes.destroy': {
    methods: ["DELETE"],
    pattern: '/schools/classes/:id',
    tokens: [{"old":"/schools/classes/:id","type":0,"val":"schools","end":""},{"old":"/schools/classes/:id","type":0,"val":"classes","end":""},{"old":"/schools/classes/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['schools.classes.destroy']['types'],
  },
  'schools.timetable.index': {
    methods: ["GET","HEAD"],
    pattern: '/schools/timetable',
    tokens: [{"old":"/schools/timetable","type":0,"val":"schools","end":""},{"old":"/schools/timetable","type":0,"val":"timetable","end":""}],
    types: placeholder as Registry['schools.timetable.index']['types'],
  },
  'schools.timetable.create': {
    methods: ["GET","HEAD"],
    pattern: '/schools/timetable/create',
    tokens: [{"old":"/schools/timetable/create","type":0,"val":"schools","end":""},{"old":"/schools/timetable/create","type":0,"val":"timetable","end":""},{"old":"/schools/timetable/create","type":0,"val":"create","end":""}],
    types: placeholder as Registry['schools.timetable.create']['types'],
  },
  'schools.teachers.index': {
    methods: ["GET","HEAD"],
    pattern: '/schools/teachers',
    tokens: [{"old":"/schools/teachers","type":0,"val":"schools","end":""},{"old":"/schools/teachers","type":0,"val":"teachers","end":""}],
    types: placeholder as Registry['schools.teachers.index']['types'],
  },
  'schools.teachers.create': {
    methods: ["GET","HEAD"],
    pattern: '/schools/teachers/create',
    tokens: [{"old":"/schools/teachers/create","type":0,"val":"schools","end":""},{"old":"/schools/teachers/create","type":0,"val":"teachers","end":""},{"old":"/schools/teachers/create","type":0,"val":"create","end":""}],
    types: placeholder as Registry['schools.teachers.create']['types'],
  },
  'schools.teachers.store': {
    methods: ["POST"],
    pattern: '/schools/teachers',
    tokens: [{"old":"/schools/teachers","type":0,"val":"schools","end":""},{"old":"/schools/teachers","type":0,"val":"teachers","end":""}],
    types: placeholder as Registry['schools.teachers.store']['types'],
  },
  'schools.teachers.show': {
    methods: ["GET","HEAD"],
    pattern: '/schools/teachers/:id',
    tokens: [{"old":"/schools/teachers/:id","type":0,"val":"schools","end":""},{"old":"/schools/teachers/:id","type":0,"val":"teachers","end":""},{"old":"/schools/teachers/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['schools.teachers.show']['types'],
  },
  'schools.teachers.edit': {
    methods: ["GET","HEAD"],
    pattern: '/schools/teachers/:id/edit',
    tokens: [{"old":"/schools/teachers/:id/edit","type":0,"val":"schools","end":""},{"old":"/schools/teachers/:id/edit","type":0,"val":"teachers","end":""},{"old":"/schools/teachers/:id/edit","type":1,"val":"id","end":""},{"old":"/schools/teachers/:id/edit","type":0,"val":"edit","end":""}],
    types: placeholder as Registry['schools.teachers.edit']['types'],
  },
  'schools.teachers.schedule': {
    methods: ["GET","HEAD"],
    pattern: '/schools/teachers/:id/schedule',
    tokens: [{"old":"/schools/teachers/:id/schedule","type":0,"val":"schools","end":""},{"old":"/schools/teachers/:id/schedule","type":0,"val":"teachers","end":""},{"old":"/schools/teachers/:id/schedule","type":1,"val":"id","end":""},{"old":"/schools/teachers/:id/schedule","type":0,"val":"schedule","end":""}],
    types: placeholder as Registry['schools.teachers.schedule']['types'],
  },
  'schools.teachers.update': {
    methods: ["PUT"],
    pattern: '/schools/teachers/:id',
    tokens: [{"old":"/schools/teachers/:id","type":0,"val":"schools","end":""},{"old":"/schools/teachers/:id","type":0,"val":"teachers","end":""},{"old":"/schools/teachers/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['schools.teachers.update']['types'],
  },
  'schools.teachers.destroy': {
    methods: ["DELETE"],
    pattern: '/schools/teachers/:id',
    tokens: [{"old":"/schools/teachers/:id","type":0,"val":"schools","end":""},{"old":"/schools/teachers/:id","type":0,"val":"teachers","end":""},{"old":"/schools/teachers/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['schools.teachers.destroy']['types'],
  },
  'discipline.dashboard': {
    methods: ["GET","HEAD"],
    pattern: '/discipline',
    tokens: [{"old":"/discipline","type":0,"val":"discipline","end":""}],
    types: placeholder as Registry['discipline.dashboard']['types'],
  },
  'discipline.incidents.index': {
    methods: ["GET","HEAD"],
    pattern: '/discipline/incidents',
    tokens: [{"old":"/discipline/incidents","type":0,"val":"discipline","end":""},{"old":"/discipline/incidents","type":0,"val":"incidents","end":""}],
    types: placeholder as Registry['discipline.incidents.index']['types'],
  },
  'discipline.incidents.report': {
    methods: ["GET","HEAD"],
    pattern: '/discipline/incidents/report',
    tokens: [{"old":"/discipline/incidents/report","type":0,"val":"discipline","end":""},{"old":"/discipline/incidents/report","type":0,"val":"incidents","end":""},{"old":"/discipline/incidents/report","type":0,"val":"report","end":""}],
    types: placeholder as Registry['discipline.incidents.report']['types'],
  },
  'discipline.incidents.store': {
    methods: ["POST"],
    pattern: '/discipline/incidents/report',
    tokens: [{"old":"/discipline/incidents/report","type":0,"val":"discipline","end":""},{"old":"/discipline/incidents/report","type":0,"val":"incidents","end":""},{"old":"/discipline/incidents/report","type":0,"val":"report","end":""}],
    types: placeholder as Registry['discipline.incidents.store']['types'],
  },
  'discipline.incidents.show': {
    methods: ["GET","HEAD"],
    pattern: '/discipline/incidents/:id/show',
    tokens: [{"old":"/discipline/incidents/:id/show","type":0,"val":"discipline","end":""},{"old":"/discipline/incidents/:id/show","type":0,"val":"incidents","end":""},{"old":"/discipline/incidents/:id/show","type":1,"val":"id","end":""},{"old":"/discipline/incidents/:id/show","type":0,"val":"show","end":""}],
    types: placeholder as Registry['discipline.incidents.show']['types'],
  },
  'discipline.incidents.edit': {
    methods: ["GET","HEAD"],
    pattern: '/discipline/incidents/:id/edit',
    tokens: [{"old":"/discipline/incidents/:id/edit","type":0,"val":"discipline","end":""},{"old":"/discipline/incidents/:id/edit","type":0,"val":"incidents","end":""},{"old":"/discipline/incidents/:id/edit","type":1,"val":"id","end":""},{"old":"/discipline/incidents/:id/edit","type":0,"val":"edit","end":""}],
    types: placeholder as Registry['discipline.incidents.edit']['types'],
  },
  'discipline.incidents.update': {
    methods: ["PUT"],
    pattern: '/discipline/incidents/:id/update',
    tokens: [{"old":"/discipline/incidents/:id/update","type":0,"val":"discipline","end":""},{"old":"/discipline/incidents/:id/update","type":0,"val":"incidents","end":""},{"old":"/discipline/incidents/:id/update","type":1,"val":"id","end":""},{"old":"/discipline/incidents/:id/update","type":0,"val":"update","end":""}],
    types: placeholder as Registry['discipline.incidents.update']['types'],
  },
  'discipline.incidents.update.post': {
    methods: ["POST"],
    pattern: '/discipline/incidents/:id/update',
    tokens: [{"old":"/discipline/incidents/:id/update","type":0,"val":"discipline","end":""},{"old":"/discipline/incidents/:id/update","type":0,"val":"incidents","end":""},{"old":"/discipline/incidents/:id/update","type":1,"val":"id","end":""},{"old":"/discipline/incidents/:id/update","type":0,"val":"update","end":""}],
    types: placeholder as Registry['discipline.incidents.update.post']['types'],
  },
  'discipline.incidents.destroy': {
    methods: ["DELETE"],
    pattern: '/discipline/incidents/:id',
    tokens: [{"old":"/discipline/incidents/:id","type":0,"val":"discipline","end":""},{"old":"/discipline/incidents/:id","type":0,"val":"incidents","end":""},{"old":"/discipline/incidents/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['discipline.incidents.destroy']['types'],
  },
  'discipline.students.index': {
    methods: ["GET","HEAD"],
    pattern: '/discipline/students',
    tokens: [{"old":"/discipline/students","type":0,"val":"discipline","end":""},{"old":"/discipline/students","type":0,"val":"students","end":""}],
    types: placeholder as Registry['discipline.students.index']['types'],
  },
  'discipline.students.show': {
    methods: ["GET","HEAD"],
    pattern: '/discipline/students/:id',
    tokens: [{"old":"/discipline/students/:id","type":0,"val":"discipline","end":""},{"old":"/discipline/students/:id","type":0,"val":"students","end":""},{"old":"/discipline/students/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['discipline.students.show']['types'],
  },
  'discipline.sanctions.apply': {
    methods: ["GET","HEAD"],
    pattern: '/discipline/sanctions/apply',
    tokens: [{"old":"/discipline/sanctions/apply","type":0,"val":"discipline","end":""},{"old":"/discipline/sanctions/apply","type":0,"val":"sanctions","end":""},{"old":"/discipline/sanctions/apply","type":0,"val":"apply","end":""}],
    types: placeholder as Registry['discipline.sanctions.apply']['types'],
  },
  'discipline.sanctions.apply.store': {
    methods: ["POST"],
    pattern: '/discipline/sanctions/apply',
    tokens: [{"old":"/discipline/sanctions/apply","type":0,"val":"discipline","end":""},{"old":"/discipline/sanctions/apply","type":0,"val":"sanctions","end":""},{"old":"/discipline/sanctions/apply","type":0,"val":"apply","end":""}],
    types: placeholder as Registry['discipline.sanctions.apply.store']['types'],
  },
  'academic.calendar': {
    methods: ["GET","HEAD"],
    pattern: '/academic/calendar',
    tokens: [{"old":"/academic/calendar","type":0,"val":"academic","end":""},{"old":"/academic/calendar","type":0,"val":"calendar","end":""}],
    types: placeholder as Registry['academic.calendar']['types'],
  },
  'financial.index': {
    methods: ["GET","HEAD"],
    pattern: '/financial',
    tokens: [{"old":"/financial","type":0,"val":"financial","end":""}],
    types: placeholder as Registry['financial.index']['types'],
  },
  'financial.fees.index': {
    methods: ["GET","HEAD"],
    pattern: '/financial/fees',
    tokens: [{"old":"/financial/fees","type":0,"val":"financial","end":""},{"old":"/financial/fees","type":0,"val":"fees","end":""}],
    types: placeholder as Registry['financial.fees.index']['types'],
  },
  'financial.fees.create': {
    methods: ["GET","HEAD"],
    pattern: '/financial/fees/create',
    tokens: [{"old":"/financial/fees/create","type":0,"val":"financial","end":""},{"old":"/financial/fees/create","type":0,"val":"fees","end":""},{"old":"/financial/fees/create","type":0,"val":"create","end":""}],
    types: placeholder as Registry['financial.fees.create']['types'],
  },
  'financial.fees.store': {
    methods: ["POST"],
    pattern: '/financial/fees/create',
    tokens: [{"old":"/financial/fees/create","type":0,"val":"financial","end":""},{"old":"/financial/fees/create","type":0,"val":"fees","end":""},{"old":"/financial/fees/create","type":0,"val":"create","end":""}],
    types: placeholder as Registry['financial.fees.store']['types'],
  },
  'financial.fees.structure': {
    methods: ["GET","HEAD"],
    pattern: '/financial/fees/structure',
    tokens: [{"old":"/financial/fees/structure","type":0,"val":"financial","end":""},{"old":"/financial/fees/structure","type":0,"val":"fees","end":""},{"old":"/financial/fees/structure","type":0,"val":"structure","end":""}],
    types: placeholder as Registry['financial.fees.structure']['types'],
  },
  'financial.fees.edit': {
    methods: ["GET","HEAD"],
    pattern: '/financial/fees/:id/edit',
    tokens: [{"old":"/financial/fees/:id/edit","type":0,"val":"financial","end":""},{"old":"/financial/fees/:id/edit","type":0,"val":"fees","end":""},{"old":"/financial/fees/:id/edit","type":1,"val":"id","end":""},{"old":"/financial/fees/:id/edit","type":0,"val":"edit","end":""}],
    types: placeholder as Registry['financial.fees.edit']['types'],
  },
  'financial.fees.update': {
    methods: ["PUT"],
    pattern: '/financial/fees/:id/update',
    tokens: [{"old":"/financial/fees/:id/update","type":0,"val":"financial","end":""},{"old":"/financial/fees/:id/update","type":0,"val":"fees","end":""},{"old":"/financial/fees/:id/update","type":1,"val":"id","end":""},{"old":"/financial/fees/:id/update","type":0,"val":"update","end":""}],
    types: placeholder as Registry['financial.fees.update']['types'],
  },
  'financial.fees.update.post': {
    methods: ["POST"],
    pattern: '/financial/fees/:id/update',
    tokens: [{"old":"/financial/fees/:id/update","type":0,"val":"financial","end":""},{"old":"/financial/fees/:id/update","type":0,"val":"fees","end":""},{"old":"/financial/fees/:id/update","type":1,"val":"id","end":""},{"old":"/financial/fees/:id/update","type":0,"val":"update","end":""}],
    types: placeholder as Registry['financial.fees.update.post']['types'],
  },
  'financial.fees.toggle_status': {
    methods: ["POST"],
    pattern: '/financial/fees/:id/toggle-status',
    tokens: [{"old":"/financial/fees/:id/toggle-status","type":0,"val":"financial","end":""},{"old":"/financial/fees/:id/toggle-status","type":0,"val":"fees","end":""},{"old":"/financial/fees/:id/toggle-status","type":1,"val":"id","end":""},{"old":"/financial/fees/:id/toggle-status","type":0,"val":"toggle-status","end":""}],
    types: placeholder as Registry['financial.fees.toggle_status']['types'],
  },
  'financial.fees.destroy': {
    methods: ["DELETE"],
    pattern: '/financial/fees/:id',
    tokens: [{"old":"/financial/fees/:id","type":0,"val":"financial","end":""},{"old":"/financial/fees/:id","type":0,"val":"fees","end":""},{"old":"/financial/fees/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['financial.fees.destroy']['types'],
  },
  'financial.payments.index': {
    methods: ["GET","HEAD"],
    pattern: '/financial/payments',
    tokens: [{"old":"/financial/payments","type":0,"val":"financial","end":""},{"old":"/financial/payments","type":0,"val":"payments","end":""}],
    types: placeholder as Registry['financial.payments.index']['types'],
  },
  'financial.payments.record': {
    methods: ["GET","HEAD"],
    pattern: '/financial/payments/record',
    tokens: [{"old":"/financial/payments/record","type":0,"val":"financial","end":""},{"old":"/financial/payments/record","type":0,"val":"payments","end":""},{"old":"/financial/payments/record","type":0,"val":"record","end":""}],
    types: placeholder as Registry['financial.payments.record']['types'],
  },
  'financial.payments.store': {
    methods: ["POST"],
    pattern: '/financial/payments/record',
    tokens: [{"old":"/financial/payments/record","type":0,"val":"financial","end":""},{"old":"/financial/payments/record","type":0,"val":"payments","end":""},{"old":"/financial/payments/record","type":0,"val":"record","end":""}],
    types: placeholder as Registry['financial.payments.store']['types'],
  },
  'financial.payments.receipt': {
    methods: ["GET","HEAD"],
    pattern: '/financial/payments/receipt/:id',
    tokens: [{"old":"/financial/payments/receipt/:id","type":0,"val":"financial","end":""},{"old":"/financial/payments/receipt/:id","type":0,"val":"payments","end":""},{"old":"/financial/payments/receipt/:id","type":0,"val":"receipt","end":""},{"old":"/financial/payments/receipt/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['financial.payments.receipt']['types'],
  },
  'financial.payments.print': {
    methods: ["GET","HEAD"],
    pattern: '/financial/payments/print-receipt/:id',
    tokens: [{"old":"/financial/payments/print-receipt/:id","type":0,"val":"financial","end":""},{"old":"/financial/payments/print-receipt/:id","type":0,"val":"payments","end":""},{"old":"/financial/payments/print-receipt/:id","type":0,"val":"print-receipt","end":""},{"old":"/financial/payments/print-receipt/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['financial.payments.print']['types'],
  },
  'financial.payments.destroy': {
    methods: ["DELETE"],
    pattern: '/financial/payments/:id',
    tokens: [{"old":"/financial/payments/:id","type":0,"val":"financial","end":""},{"old":"/financial/payments/:id","type":0,"val":"payments","end":""},{"old":"/financial/payments/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['financial.payments.destroy']['types'],
  },
  'financial.reports.income': {
    methods: ["GET","HEAD"],
    pattern: '/financial/reports/income',
    tokens: [{"old":"/financial/reports/income","type":0,"val":"financial","end":""},{"old":"/financial/reports/income","type":0,"val":"reports","end":""},{"old":"/financial/reports/income","type":0,"val":"income","end":""}],
    types: placeholder as Registry['financial.reports.income']['types'],
  },
  'financial.reports.outstanding': {
    methods: ["GET","HEAD"],
    pattern: '/financial/reports/outstanding',
    tokens: [{"old":"/financial/reports/outstanding","type":0,"val":"financial","end":""},{"old":"/financial/reports/outstanding","type":0,"val":"reports","end":""},{"old":"/financial/reports/outstanding","type":0,"val":"outstanding","end":""}],
    types: placeholder as Registry['financial.reports.outstanding']['types'],
  },
  'financial.reports.statistics': {
    methods: ["GET","HEAD"],
    pattern: '/financial/reports/statistics',
    tokens: [{"old":"/financial/reports/statistics","type":0,"val":"financial","end":""},{"old":"/financial/reports/statistics","type":0,"val":"reports","end":""},{"old":"/financial/reports/statistics","type":0,"val":"statistics","end":""}],
    types: placeholder as Registry['financial.reports.statistics']['types'],
  },
  'financial.reports.export': {
    methods: ["GET","HEAD"],
    pattern: '/financial/reports/export',
    tokens: [{"old":"/financial/reports/export","type":0,"val":"financial","end":""},{"old":"/financial/reports/export","type":0,"val":"reports","end":""},{"old":"/financial/reports/export","type":0,"val":"export","end":""}],
    types: placeholder as Registry['financial.reports.export']['types'],
  },
  'legacy.api.students.financial_status': {
    methods: ["GET","HEAD"],
    pattern: '/api/students/:studentId/financial-status',
    tokens: [{"old":"/api/students/:studentId/financial-status","type":0,"val":"api","end":""},{"old":"/api/students/:studentId/financial-status","type":0,"val":"students","end":""},{"old":"/api/students/:studentId/financial-status","type":1,"val":"studentId","end":""},{"old":"/api/students/:studentId/financial-status","type":0,"val":"financial-status","end":""}],
    types: placeholder as Registry['legacy.api.students.financial_status']['types'],
  },
  'schools.profile.update.web': {
    methods: ["POST"],
    pattern: '/schools/profile/update',
    tokens: [{"old":"/schools/profile/update","type":0,"val":"schools","end":""},{"old":"/schools/profile/update","type":0,"val":"profile","end":""},{"old":"/schools/profile/update","type":0,"val":"update","end":""}],
    types: placeholder as Registry['schools.profile.update.web']['types'],
  },
  'teachers.attendance_index_page': {
    methods: ["GET","HEAD"],
    pattern: '/teacher/attendance',
    tokens: [{"old":"/teacher/attendance","type":0,"val":"teacher","end":""},{"old":"/teacher/attendance","type":0,"val":"attendance","end":""}],
    types: placeholder as Registry['teachers.attendance_index_page']['types'],
  },
  'teachers.attendance_mark_page': {
    methods: ["GET","HEAD"],
    pattern: '/teacher/attendance/mark',
    tokens: [{"old":"/teacher/attendance/mark","type":0,"val":"teacher","end":""},{"old":"/teacher/attendance/mark","type":0,"val":"attendance","end":""},{"old":"/teacher/attendance/mark","type":0,"val":"mark","end":""}],
    types: placeholder as Registry['teachers.attendance_mark_page']['types'],
  },
  'schools.register_school': {
    methods: ["POST"],
    pattern: '/api/v1/register-school',
    tokens: [{"old":"/api/v1/register-school","type":0,"val":"api","end":""},{"old":"/api/v1/register-school","type":0,"val":"v1","end":""},{"old":"/api/v1/register-school","type":0,"val":"register-school","end":""}],
    types: placeholder as Registry['schools.register_school']['types'],
  },
  'auth.login': {
    methods: ["POST"],
    pattern: '/api/v1/login',
    tokens: [{"old":"/api/v1/login","type":0,"val":"api","end":""},{"old":"/api/v1/login","type":0,"val":"v1","end":""},{"old":"/api/v1/login","type":0,"val":"login","end":""}],
    types: placeholder as Registry['auth.login']['types'],
  },
  'auth.request_otp': {
    methods: ["POST"],
    pattern: '/api/v1/auth/otp/request',
    tokens: [{"old":"/api/v1/auth/otp/request","type":0,"val":"api","end":""},{"old":"/api/v1/auth/otp/request","type":0,"val":"v1","end":""},{"old":"/api/v1/auth/otp/request","type":0,"val":"auth","end":""},{"old":"/api/v1/auth/otp/request","type":0,"val":"otp","end":""},{"old":"/api/v1/auth/otp/request","type":0,"val":"request","end":""}],
    types: placeholder as Registry['auth.request_otp']['types'],
  },
  'auth.verify_otp': {
    methods: ["POST"],
    pattern: '/api/v1/auth/otp/verify',
    tokens: [{"old":"/api/v1/auth/otp/verify","type":0,"val":"api","end":""},{"old":"/api/v1/auth/otp/verify","type":0,"val":"v1","end":""},{"old":"/api/v1/auth/otp/verify","type":0,"val":"auth","end":""},{"old":"/api/v1/auth/otp/verify","type":0,"val":"otp","end":""},{"old":"/api/v1/auth/otp/verify","type":0,"val":"verify","end":""}],
    types: placeholder as Registry['auth.verify_otp']['types'],
  },
  'auth.forgot_password': {
    methods: ["POST"],
    pattern: '/api/v1/forgot-password',
    tokens: [{"old":"/api/v1/forgot-password","type":0,"val":"api","end":""},{"old":"/api/v1/forgot-password","type":0,"val":"v1","end":""},{"old":"/api/v1/forgot-password","type":0,"val":"forgot-password","end":""}],
    types: placeholder as Registry['auth.forgot_password']['types'],
  },
  'auth.reset_password': {
    methods: ["POST"],
    pattern: '/api/v1/reset-password',
    tokens: [{"old":"/api/v1/reset-password","type":0,"val":"api","end":""},{"old":"/api/v1/reset-password","type":0,"val":"v1","end":""},{"old":"/api/v1/reset-password","type":0,"val":"reset-password","end":""}],
    types: placeholder as Registry['auth.reset_password']['types'],
  },
  'transfers.verify_authorization': {
    methods: ["POST"],
    pattern: '/api/v1/verify-transfer',
    tokens: [{"old":"/api/v1/verify-transfer","type":0,"val":"api","end":""},{"old":"/api/v1/verify-transfer","type":0,"val":"v1","end":""},{"old":"/api/v1/verify-transfer","type":0,"val":"verify-transfer","end":""}],
    types: placeholder as Registry['transfers.verify_authorization']['types'],
  },
  'auth.logout': {
    methods: ["POST"],
    pattern: '/api/v1/logout',
    tokens: [{"old":"/api/v1/logout","type":0,"val":"api","end":""},{"old":"/api/v1/logout","type":0,"val":"v1","end":""},{"old":"/api/v1/logout","type":0,"val":"logout","end":""}],
    types: placeholder as Registry['auth.logout']['types'],
  },
  'auth.change_password': {
    methods: ["POST"],
    pattern: '/api/v1/change-password',
    tokens: [{"old":"/api/v1/change-password","type":0,"val":"api","end":""},{"old":"/api/v1/change-password","type":0,"val":"v1","end":""},{"old":"/api/v1/change-password","type":0,"val":"change-password","end":""}],
    types: placeholder as Registry['auth.change_password']['types'],
  },
  'auth.get_profile': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/profile',
    tokens: [{"old":"/api/v1/profile","type":0,"val":"api","end":""},{"old":"/api/v1/profile","type":0,"val":"v1","end":""},{"old":"/api/v1/profile","type":0,"val":"profile","end":""}],
    types: placeholder as Registry['auth.get_profile']['types'],
  },
  'auth.update_profile': {
    methods: ["PUT"],
    pattern: '/api/v1/profile',
    tokens: [{"old":"/api/v1/profile","type":0,"val":"api","end":""},{"old":"/api/v1/profile","type":0,"val":"v1","end":""},{"old":"/api/v1/profile","type":0,"val":"profile","end":""}],
    types: placeholder as Registry['auth.update_profile']['types'],
  },
  'inspections.get_all_schools': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/inspection/schools',
    tokens: [{"old":"/api/v1/inspection/schools","type":0,"val":"api","end":""},{"old":"/api/v1/inspection/schools","type":0,"val":"v1","end":""},{"old":"/api/v1/inspection/schools","type":0,"val":"inspection","end":""},{"old":"/api/v1/inspection/schools","type":0,"val":"schools","end":""}],
    types: placeholder as Registry['inspections.get_all_schools']['types'],
  },
  'inspections.get_school_by_id': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/inspection/schools/:id',
    tokens: [{"old":"/api/v1/inspection/schools/:id","type":0,"val":"api","end":""},{"old":"/api/v1/inspection/schools/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/inspection/schools/:id","type":0,"val":"inspection","end":""},{"old":"/api/v1/inspection/schools/:id","type":0,"val":"schools","end":""},{"old":"/api/v1/inspection/schools/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['inspections.get_school_by_id']['types'],
  },
  'inspections.approve_school': {
    methods: ["POST"],
    pattern: '/api/v1/inspection/schools/approve/:id',
    tokens: [{"old":"/api/v1/inspection/schools/approve/:id","type":0,"val":"api","end":""},{"old":"/api/v1/inspection/schools/approve/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/inspection/schools/approve/:id","type":0,"val":"inspection","end":""},{"old":"/api/v1/inspection/schools/approve/:id","type":0,"val":"schools","end":""},{"old":"/api/v1/inspection/schools/approve/:id","type":0,"val":"approve","end":""},{"old":"/api/v1/inspection/schools/approve/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['inspections.approve_school']['types'],
  },
  'inspections.suspend_school': {
    methods: ["POST"],
    pattern: '/api/v1/inspection/schools/suspend/:id',
    tokens: [{"old":"/api/v1/inspection/schools/suspend/:id","type":0,"val":"api","end":""},{"old":"/api/v1/inspection/schools/suspend/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/inspection/schools/suspend/:id","type":0,"val":"inspection","end":""},{"old":"/api/v1/inspection/schools/suspend/:id","type":0,"val":"schools","end":""},{"old":"/api/v1/inspection/schools/suspend/:id","type":0,"val":"suspend","end":""},{"old":"/api/v1/inspection/schools/suspend/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['inspections.suspend_school']['types'],
  },
  'inspections.inspect_school': {
    methods: ["POST"],
    pattern: '/api/v1/inspection/schools/inspect',
    tokens: [{"old":"/api/v1/inspection/schools/inspect","type":0,"val":"api","end":""},{"old":"/api/v1/inspection/schools/inspect","type":0,"val":"v1","end":""},{"old":"/api/v1/inspection/schools/inspect","type":0,"val":"inspection","end":""},{"old":"/api/v1/inspection/schools/inspect","type":0,"val":"schools","end":""},{"old":"/api/v1/inspection/schools/inspect","type":0,"val":"inspect","end":""}],
    types: placeholder as Registry['inspections.inspect_school']['types'],
  },
  'auth.generate_school_credentials': {
    methods: ["POST"],
    pattern: '/api/v1/inspection/schools/generate-credentials/:id',
    tokens: [{"old":"/api/v1/inspection/schools/generate-credentials/:id","type":0,"val":"api","end":""},{"old":"/api/v1/inspection/schools/generate-credentials/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/inspection/schools/generate-credentials/:id","type":0,"val":"inspection","end":""},{"old":"/api/v1/inspection/schools/generate-credentials/:id","type":0,"val":"schools","end":""},{"old":"/api/v1/inspection/schools/generate-credentials/:id","type":0,"val":"generate-credentials","end":""},{"old":"/api/v1/inspection/schools/generate-credentials/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['auth.generate_school_credentials']['types'],
  },
  'inspections.send_global_communication': {
    methods: ["POST"],
    pattern: '/api/v1/inspection/communications/global',
    tokens: [{"old":"/api/v1/inspection/communications/global","type":0,"val":"api","end":""},{"old":"/api/v1/inspection/communications/global","type":0,"val":"v1","end":""},{"old":"/api/v1/inspection/communications/global","type":0,"val":"inspection","end":""},{"old":"/api/v1/inspection/communications/global","type":0,"val":"communications","end":""},{"old":"/api/v1/inspection/communications/global","type":0,"val":"global","end":""}],
    types: placeholder as Registry['inspections.send_global_communication']['types'],
  },
  'inspection.send_school_communication': {
    methods: ["POST"],
    pattern: '/api/v1/inspection/communications/school',
    tokens: [{"old":"/api/v1/inspection/communications/school","type":0,"val":"api","end":""},{"old":"/api/v1/inspection/communications/school","type":0,"val":"v1","end":""},{"old":"/api/v1/inspection/communications/school","type":0,"val":"inspection","end":""},{"old":"/api/v1/inspection/communications/school","type":0,"val":"communications","end":""},{"old":"/api/v1/inspection/communications/school","type":0,"val":"school","end":""}],
    types: placeholder as Registry['inspection.send_school_communication']['types'],
  },
  'inspections.get_global_stats': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/inspection/stats/global',
    tokens: [{"old":"/api/v1/inspection/stats/global","type":0,"val":"api","end":""},{"old":"/api/v1/inspection/stats/global","type":0,"val":"v1","end":""},{"old":"/api/v1/inspection/stats/global","type":0,"val":"inspection","end":""},{"old":"/api/v1/inspection/stats/global","type":0,"val":"stats","end":""},{"old":"/api/v1/inspection/stats/global","type":0,"val":"global","end":""}],
    types: placeholder as Registry['inspections.get_global_stats']['types'],
  },
  'inspections.generate_school_report': {
    methods: ["POST"],
    pattern: '/api/v1/inspection/reports/school',
    tokens: [{"old":"/api/v1/inspection/reports/school","type":0,"val":"api","end":""},{"old":"/api/v1/inspection/reports/school","type":0,"val":"v1","end":""},{"old":"/api/v1/inspection/reports/school","type":0,"val":"inspection","end":""},{"old":"/api/v1/inspection/reports/school","type":0,"val":"reports","end":""},{"old":"/api/v1/inspection/reports/school","type":0,"val":"school","end":""}],
    types: placeholder as Registry['inspections.generate_school_report']['types'],
  },
  'schools.dashboard': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/school/dashboard',
    tokens: [{"old":"/api/v1/school/dashboard","type":0,"val":"api","end":""},{"old":"/api/v1/school/dashboard","type":0,"val":"v1","end":""},{"old":"/api/v1/school/dashboard","type":0,"val":"school","end":""},{"old":"/api/v1/school/dashboard","type":0,"val":"dashboard","end":""}],
    types: placeholder as Registry['schools.dashboard']['types'],
  },
  'schools.update_school_profile': {
    methods: ["PUT"],
    pattern: '/api/v1/school/profile',
    tokens: [{"old":"/api/v1/school/profile","type":0,"val":"api","end":""},{"old":"/api/v1/school/profile","type":0,"val":"v1","end":""},{"old":"/api/v1/school/profile","type":0,"val":"school","end":""},{"old":"/api/v1/school/profile","type":0,"val":"profile","end":""}],
    types: placeholder as Registry['schools.update_school_profile']['types'],
  },
  'academics.get_classes': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/school/classes',
    tokens: [{"old":"/api/v1/school/classes","type":0,"val":"api","end":""},{"old":"/api/v1/school/classes","type":0,"val":"v1","end":""},{"old":"/api/v1/school/classes","type":0,"val":"school","end":""},{"old":"/api/v1/school/classes","type":0,"val":"classes","end":""}],
    types: placeholder as Registry['academics.get_classes']['types'],
  },
  'academics.create_class': {
    methods: ["POST"],
    pattern: '/api/v1/school/classes',
    tokens: [{"old":"/api/v1/school/classes","type":0,"val":"api","end":""},{"old":"/api/v1/school/classes","type":0,"val":"v1","end":""},{"old":"/api/v1/school/classes","type":0,"val":"school","end":""},{"old":"/api/v1/school/classes","type":0,"val":"classes","end":""}],
    types: placeholder as Registry['academics.create_class']['types'],
  },
  'academics.get_class_by_id': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/school/classes/:id',
    tokens: [{"old":"/api/v1/school/classes/:id","type":0,"val":"api","end":""},{"old":"/api/v1/school/classes/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/school/classes/:id","type":0,"val":"school","end":""},{"old":"/api/v1/school/classes/:id","type":0,"val":"classes","end":""},{"old":"/api/v1/school/classes/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['academics.get_class_by_id']['types'],
  },
  'academics.update_class': {
    methods: ["PUT"],
    pattern: '/api/v1/school/classes/:id',
    tokens: [{"old":"/api/v1/school/classes/:id","type":0,"val":"api","end":""},{"old":"/api/v1/school/classes/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/school/classes/:id","type":0,"val":"school","end":""},{"old":"/api/v1/school/classes/:id","type":0,"val":"classes","end":""},{"old":"/api/v1/school/classes/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['academics.update_class']['types'],
  },
  'academics.delete_class': {
    methods: ["DELETE"],
    pattern: '/api/v1/school/classes/:id',
    tokens: [{"old":"/api/v1/school/classes/:id","type":0,"val":"api","end":""},{"old":"/api/v1/school/classes/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/school/classes/:id","type":0,"val":"school","end":""},{"old":"/api/v1/school/classes/:id","type":0,"val":"classes","end":""},{"old":"/api/v1/school/classes/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['academics.delete_class']['types'],
  },
  'academics.get_class_students': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/school/classes/:id/students',
    tokens: [{"old":"/api/v1/school/classes/:id/students","type":0,"val":"api","end":""},{"old":"/api/v1/school/classes/:id/students","type":0,"val":"v1","end":""},{"old":"/api/v1/school/classes/:id/students","type":0,"val":"school","end":""},{"old":"/api/v1/school/classes/:id/students","type":0,"val":"classes","end":""},{"old":"/api/v1/school/classes/:id/students","type":1,"val":"id","end":""},{"old":"/api/v1/school/classes/:id/students","type":0,"val":"students","end":""}],
    types: placeholder as Registry['academics.get_class_students']['types'],
  },
  'academics.get_subjects': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/school/subjects',
    tokens: [{"old":"/api/v1/school/subjects","type":0,"val":"api","end":""},{"old":"/api/v1/school/subjects","type":0,"val":"v1","end":""},{"old":"/api/v1/school/subjects","type":0,"val":"school","end":""},{"old":"/api/v1/school/subjects","type":0,"val":"subjects","end":""}],
    types: placeholder as Registry['academics.get_subjects']['types'],
  },
  'academics.create_subject': {
    methods: ["POST"],
    pattern: '/api/v1/school/subjects',
    tokens: [{"old":"/api/v1/school/subjects","type":0,"val":"api","end":""},{"old":"/api/v1/school/subjects","type":0,"val":"v1","end":""},{"old":"/api/v1/school/subjects","type":0,"val":"school","end":""},{"old":"/api/v1/school/subjects","type":0,"val":"subjects","end":""}],
    types: placeholder as Registry['academics.create_subject']['types'],
  },
  'academics.update_subject': {
    methods: ["PUT"],
    pattern: '/api/v1/school/subjects/:id',
    tokens: [{"old":"/api/v1/school/subjects/:id","type":0,"val":"api","end":""},{"old":"/api/v1/school/subjects/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/school/subjects/:id","type":0,"val":"school","end":""},{"old":"/api/v1/school/subjects/:id","type":0,"val":"subjects","end":""},{"old":"/api/v1/school/subjects/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['academics.update_subject']['types'],
  },
  'academics.delete_subject': {
    methods: ["DELETE"],
    pattern: '/api/v1/school/subjects/:id',
    tokens: [{"old":"/api/v1/school/subjects/:id","type":0,"val":"api","end":""},{"old":"/api/v1/school/subjects/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/school/subjects/:id","type":0,"val":"school","end":""},{"old":"/api/v1/school/subjects/:id","type":0,"val":"subjects","end":""},{"old":"/api/v1/school/subjects/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['academics.delete_subject']['types'],
  },
  'academics.get_class_subjects': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/school/classes/:classId/subjects',
    tokens: [{"old":"/api/v1/school/classes/:classId/subjects","type":0,"val":"api","end":""},{"old":"/api/v1/school/classes/:classId/subjects","type":0,"val":"v1","end":""},{"old":"/api/v1/school/classes/:classId/subjects","type":0,"val":"school","end":""},{"old":"/api/v1/school/classes/:classId/subjects","type":0,"val":"classes","end":""},{"old":"/api/v1/school/classes/:classId/subjects","type":1,"val":"classId","end":""},{"old":"/api/v1/school/classes/:classId/subjects","type":0,"val":"subjects","end":""}],
    types: placeholder as Registry['academics.get_class_subjects']['types'],
  },
  'academics.add_subject_to_class': {
    methods: ["POST"],
    pattern: '/api/v1/school/classes/:classId/subjects',
    tokens: [{"old":"/api/v1/school/classes/:classId/subjects","type":0,"val":"api","end":""},{"old":"/api/v1/school/classes/:classId/subjects","type":0,"val":"v1","end":""},{"old":"/api/v1/school/classes/:classId/subjects","type":0,"val":"school","end":""},{"old":"/api/v1/school/classes/:classId/subjects","type":0,"val":"classes","end":""},{"old":"/api/v1/school/classes/:classId/subjects","type":1,"val":"classId","end":""},{"old":"/api/v1/school/classes/:classId/subjects","type":0,"val":"subjects","end":""}],
    types: placeholder as Registry['academics.add_subject_to_class']['types'],
  },
  'academics.get_grades_by_class': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/school/classes/:classId/grades',
    tokens: [{"old":"/api/v1/school/classes/:classId/grades","type":0,"val":"api","end":""},{"old":"/api/v1/school/classes/:classId/grades","type":0,"val":"v1","end":""},{"old":"/api/v1/school/classes/:classId/grades","type":0,"val":"school","end":""},{"old":"/api/v1/school/classes/:classId/grades","type":0,"val":"classes","end":""},{"old":"/api/v1/school/classes/:classId/grades","type":1,"val":"classId","end":""},{"old":"/api/v1/school/classes/:classId/grades","type":0,"val":"grades","end":""}],
    types: placeholder as Registry['academics.get_grades_by_class']['types'],
  },
  'academics.get_grades_by_student': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/school/students/:studentId/grades',
    tokens: [{"old":"/api/v1/school/students/:studentId/grades","type":0,"val":"api","end":""},{"old":"/api/v1/school/students/:studentId/grades","type":0,"val":"v1","end":""},{"old":"/api/v1/school/students/:studentId/grades","type":0,"val":"school","end":""},{"old":"/api/v1/school/students/:studentId/grades","type":0,"val":"students","end":""},{"old":"/api/v1/school/students/:studentId/grades","type":1,"val":"studentId","end":""},{"old":"/api/v1/school/students/:studentId/grades","type":0,"val":"grades","end":""}],
    types: placeholder as Registry['academics.get_grades_by_student']['types'],
  },
  'academics.add_grade': {
    methods: ["POST"],
    pattern: '/api/v1/school/grades',
    tokens: [{"old":"/api/v1/school/grades","type":0,"val":"api","end":""},{"old":"/api/v1/school/grades","type":0,"val":"v1","end":""},{"old":"/api/v1/school/grades","type":0,"val":"school","end":""},{"old":"/api/v1/school/grades","type":0,"val":"grades","end":""}],
    types: placeholder as Registry['academics.add_grade']['types'],
  },
  'academics.update_grade': {
    methods: ["PUT"],
    pattern: '/api/v1/school/grades/:id',
    tokens: [{"old":"/api/v1/school/grades/:id","type":0,"val":"api","end":""},{"old":"/api/v1/school/grades/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/school/grades/:id","type":0,"val":"school","end":""},{"old":"/api/v1/school/grades/:id","type":0,"val":"grades","end":""},{"old":"/api/v1/school/grades/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['academics.update_grade']['types'],
  },
  'academics.delete_grade': {
    methods: ["DELETE"],
    pattern: '/api/v1/school/grades/:id',
    tokens: [{"old":"/api/v1/school/grades/:id","type":0,"val":"api","end":""},{"old":"/api/v1/school/grades/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/school/grades/:id","type":0,"val":"school","end":""},{"old":"/api/v1/school/grades/:id","type":0,"val":"grades","end":""},{"old":"/api/v1/school/grades/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['academics.delete_grade']['types'],
  },
  'academics.publish_grades': {
    methods: ["POST"],
    pattern: '/api/v1/school/grades/publish',
    tokens: [{"old":"/api/v1/school/grades/publish","type":0,"val":"api","end":""},{"old":"/api/v1/school/grades/publish","type":0,"val":"v1","end":""},{"old":"/api/v1/school/grades/publish","type":0,"val":"school","end":""},{"old":"/api/v1/school/grades/publish","type":0,"val":"grades","end":""},{"old":"/api/v1/school/grades/publish","type":0,"val":"publish","end":""}],
    types: placeholder as Registry['academics.publish_grades']['types'],
  },
  'schools.add_teacher': {
    methods: ["POST"],
    pattern: '/api/v1/school/teachers',
    tokens: [{"old":"/api/v1/school/teachers","type":0,"val":"api","end":""},{"old":"/api/v1/school/teachers","type":0,"val":"v1","end":""},{"old":"/api/v1/school/teachers","type":0,"val":"school","end":""},{"old":"/api/v1/school/teachers","type":0,"val":"teachers","end":""}],
    types: placeholder as Registry['schools.add_teacher']['types'],
  },
  'transfers.request_transfer': {
    methods: ["POST"],
    pattern: '/api/v1/school/transfers/request',
    tokens: [{"old":"/api/v1/school/transfers/request","type":0,"val":"api","end":""},{"old":"/api/v1/school/transfers/request","type":0,"val":"v1","end":""},{"old":"/api/v1/school/transfers/request","type":0,"val":"school","end":""},{"old":"/api/v1/school/transfers/request","type":0,"val":"transfers","end":""},{"old":"/api/v1/school/transfers/request","type":0,"val":"request","end":""}],
    types: placeholder as Registry['transfers.request_transfer']['types'],
  },
  'transfers.get_pending_transfers': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/school/transfers/pending',
    tokens: [{"old":"/api/v1/school/transfers/pending","type":0,"val":"api","end":""},{"old":"/api/v1/school/transfers/pending","type":0,"val":"v1","end":""},{"old":"/api/v1/school/transfers/pending","type":0,"val":"school","end":""},{"old":"/api/v1/school/transfers/pending","type":0,"val":"transfers","end":""},{"old":"/api/v1/school/transfers/pending","type":0,"val":"pending","end":""}],
    types: placeholder as Registry['transfers.get_pending_transfers']['types'],
  },
  'transfers.approve_transfer': {
    methods: ["POST"],
    pattern: '/api/v1/school/transfers/approve',
    tokens: [{"old":"/api/v1/school/transfers/approve","type":0,"val":"api","end":""},{"old":"/api/v1/school/transfers/approve","type":0,"val":"v1","end":""},{"old":"/api/v1/school/transfers/approve","type":0,"val":"school","end":""},{"old":"/api/v1/school/transfers/approve","type":0,"val":"transfers","end":""},{"old":"/api/v1/school/transfers/approve","type":0,"val":"approve","end":""}],
    types: placeholder as Registry['transfers.approve_transfer']['types'],
  },
  'transfers.reject_transfer': {
    methods: ["POST"],
    pattern: '/api/v1/school/transfers/reject',
    tokens: [{"old":"/api/v1/school/transfers/reject","type":0,"val":"api","end":""},{"old":"/api/v1/school/transfers/reject","type":0,"val":"v1","end":""},{"old":"/api/v1/school/transfers/reject","type":0,"val":"school","end":""},{"old":"/api/v1/school/transfers/reject","type":0,"val":"transfers","end":""},{"old":"/api/v1/school/transfers/reject","type":0,"val":"reject","end":""}],
    types: placeholder as Registry['transfers.reject_transfer']['types'],
  },
  'transfers.complete_transfer': {
    methods: ["POST"],
    pattern: '/api/v1/school/transfers/:id/complete',
    tokens: [{"old":"/api/v1/school/transfers/:id/complete","type":0,"val":"api","end":""},{"old":"/api/v1/school/transfers/:id/complete","type":0,"val":"v1","end":""},{"old":"/api/v1/school/transfers/:id/complete","type":0,"val":"school","end":""},{"old":"/api/v1/school/transfers/:id/complete","type":0,"val":"transfers","end":""},{"old":"/api/v1/school/transfers/:id/complete","type":1,"val":"id","end":""},{"old":"/api/v1/school/transfers/:id/complete","type":0,"val":"complete","end":""}],
    types: placeholder as Registry['transfers.complete_transfer']['types'],
  },
  'academics.get_academic_stats': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/school/stats/academic',
    tokens: [{"old":"/api/v1/school/stats/academic","type":0,"val":"api","end":""},{"old":"/api/v1/school/stats/academic","type":0,"val":"v1","end":""},{"old":"/api/v1/school/stats/academic","type":0,"val":"school","end":""},{"old":"/api/v1/school/stats/academic","type":0,"val":"stats","end":""},{"old":"/api/v1/school/stats/academic","type":0,"val":"academic","end":""}],
    types: placeholder as Registry['academics.get_academic_stats']['types'],
  },
  'academics.get_progress_stats': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/school/stats/progress',
    tokens: [{"old":"/api/v1/school/stats/progress","type":0,"val":"api","end":""},{"old":"/api/v1/school/stats/progress","type":0,"val":"v1","end":""},{"old":"/api/v1/school/stats/progress","type":0,"val":"school","end":""},{"old":"/api/v1/school/stats/progress","type":0,"val":"stats","end":""},{"old":"/api/v1/school/stats/progress","type":0,"val":"progress","end":""}],
    types: placeholder as Registry['academics.get_progress_stats']['types'],
  },
  'pedagogicals.generate_report_card': {
    methods: ["POST"],
    pattern: '/api/v1/pedagogical/report-cards/student',
    tokens: [{"old":"/api/v1/pedagogical/report-cards/student","type":0,"val":"api","end":""},{"old":"/api/v1/pedagogical/report-cards/student","type":0,"val":"v1","end":""},{"old":"/api/v1/pedagogical/report-cards/student","type":0,"val":"pedagogical","end":""},{"old":"/api/v1/pedagogical/report-cards/student","type":0,"val":"report-cards","end":""},{"old":"/api/v1/pedagogical/report-cards/student","type":0,"val":"student","end":""}],
    types: placeholder as Registry['pedagogicals.generate_report_card']['types'],
  },
  'pedagogicals.get_class_timetable': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/pedagogical/timetable/class/:classId',
    tokens: [{"old":"/api/v1/pedagogical/timetable/class/:classId","type":0,"val":"api","end":""},{"old":"/api/v1/pedagogical/timetable/class/:classId","type":0,"val":"v1","end":""},{"old":"/api/v1/pedagogical/timetable/class/:classId","type":0,"val":"pedagogical","end":""},{"old":"/api/v1/pedagogical/timetable/class/:classId","type":0,"val":"timetable","end":""},{"old":"/api/v1/pedagogical/timetable/class/:classId","type":0,"val":"class","end":""},{"old":"/api/v1/pedagogical/timetable/class/:classId","type":1,"val":"classId","end":""}],
    types: placeholder as Registry['pedagogicals.get_class_timetable']['types'],
  },
  'pedagogicals.create_timetable': {
    methods: ["POST"],
    pattern: '/api/v1/pedagogical/timetable',
    tokens: [{"old":"/api/v1/pedagogical/timetable","type":0,"val":"api","end":""},{"old":"/api/v1/pedagogical/timetable","type":0,"val":"v1","end":""},{"old":"/api/v1/pedagogical/timetable","type":0,"val":"pedagogical","end":""},{"old":"/api/v1/pedagogical/timetable","type":0,"val":"timetable","end":""}],
    types: placeholder as Registry['pedagogicals.create_timetable']['types'],
  },
  'pedagogicals.publish_grades': {
    methods: ["POST"],
    pattern: '/api/v1/pedagogical/grades/publish',
    tokens: [{"old":"/api/v1/pedagogical/grades/publish","type":0,"val":"api","end":""},{"old":"/api/v1/pedagogical/grades/publish","type":0,"val":"v1","end":""},{"old":"/api/v1/pedagogical/grades/publish","type":0,"val":"pedagogical","end":""},{"old":"/api/v1/pedagogical/grades/publish","type":0,"val":"grades","end":""},{"old":"/api/v1/pedagogical/grades/publish","type":0,"val":"publish","end":""}],
    types: placeholder as Registry['pedagogicals.publish_grades']['types'],
  },
  'pedagogicals.create_academic_calendar': {
    methods: ["POST"],
    pattern: '/api/v1/pedagogical/calendar',
    tokens: [{"old":"/api/v1/pedagogical/calendar","type":0,"val":"api","end":""},{"old":"/api/v1/pedagogical/calendar","type":0,"val":"v1","end":""},{"old":"/api/v1/pedagogical/calendar","type":0,"val":"pedagogical","end":""},{"old":"/api/v1/pedagogical/calendar","type":0,"val":"calendar","end":""}],
    types: placeholder as Registry['pedagogicals.create_academic_calendar']['types'],
  },
  'pedagogicals.create_exam_schedule': {
    methods: ["POST"],
    pattern: '/api/v1/pedagogical/exam-schedules',
    tokens: [{"old":"/api/v1/pedagogical/exam-schedules","type":0,"val":"api","end":""},{"old":"/api/v1/pedagogical/exam-schedules","type":0,"val":"v1","end":""},{"old":"/api/v1/pedagogical/exam-schedules","type":0,"val":"pedagogical","end":""},{"old":"/api/v1/pedagogical/exam-schedules","type":0,"val":"exam-schedules","end":""}],
    types: placeholder as Registry['pedagogicals.create_exam_schedule']['types'],
  },
  'pedagogicals.get_student_progress': {
    methods: ["POST"],
    pattern: '/api/v1/pedagogical/students/progress',
    tokens: [{"old":"/api/v1/pedagogical/students/progress","type":0,"val":"api","end":""},{"old":"/api/v1/pedagogical/students/progress","type":0,"val":"v1","end":""},{"old":"/api/v1/pedagogical/students/progress","type":0,"val":"pedagogical","end":""},{"old":"/api/v1/pedagogical/students/progress","type":0,"val":"students","end":""},{"old":"/api/v1/pedagogical/students/progress","type":0,"val":"progress","end":""}],
    types: placeholder as Registry['pedagogicals.get_student_progress']['types'],
  },
  'financials.get_fees': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/financial/fees',
    tokens: [{"old":"/api/v1/financial/fees","type":0,"val":"api","end":""},{"old":"/api/v1/financial/fees","type":0,"val":"v1","end":""},{"old":"/api/v1/financial/fees","type":0,"val":"financial","end":""},{"old":"/api/v1/financial/fees","type":0,"val":"fees","end":""}],
    types: placeholder as Registry['financials.get_fees']['types'],
  },
  'financials.set_fees': {
    methods: ["POST"],
    pattern: '/api/v1/financial/fees',
    tokens: [{"old":"/api/v1/financial/fees","type":0,"val":"api","end":""},{"old":"/api/v1/financial/fees","type":0,"val":"v1","end":""},{"old":"/api/v1/financial/fees","type":0,"val":"financial","end":""},{"old":"/api/v1/financial/fees","type":0,"val":"fees","end":""}],
    types: placeholder as Registry['financials.set_fees']['types'],
  },
  'financials.update_fees': {
    methods: ["PUT"],
    pattern: '/api/v1/financial/fees/:id',
    tokens: [{"old":"/api/v1/financial/fees/:id","type":0,"val":"api","end":""},{"old":"/api/v1/financial/fees/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/financial/fees/:id","type":0,"val":"financial","end":""},{"old":"/api/v1/financial/fees/:id","type":0,"val":"fees","end":""},{"old":"/api/v1/financial/fees/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['financials.update_fees']['types'],
  },
  'financials.delete_fees': {
    methods: ["DELETE"],
    pattern: '/api/v1/financial/fees/:id',
    tokens: [{"old":"/api/v1/financial/fees/:id","type":0,"val":"api","end":""},{"old":"/api/v1/financial/fees/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/financial/fees/:id","type":0,"val":"financial","end":""},{"old":"/api/v1/financial/fees/:id","type":0,"val":"fees","end":""},{"old":"/api/v1/financial/fees/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['financials.delete_fees']['types'],
  },
  'financials.record_payment': {
    methods: ["POST"],
    pattern: '/api/v1/financial/payments',
    tokens: [{"old":"/api/v1/financial/payments","type":0,"val":"api","end":""},{"old":"/api/v1/financial/payments","type":0,"val":"v1","end":""},{"old":"/api/v1/financial/payments","type":0,"val":"financial","end":""},{"old":"/api/v1/financial/payments","type":0,"val":"payments","end":""}],
    types: placeholder as Registry['financials.record_payment']['types'],
  },
  'financials.get_student_payments': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/financial/payments/student/:studentId',
    tokens: [{"old":"/api/v1/financial/payments/student/:studentId","type":0,"val":"api","end":""},{"old":"/api/v1/financial/payments/student/:studentId","type":0,"val":"v1","end":""},{"old":"/api/v1/financial/payments/student/:studentId","type":0,"val":"financial","end":""},{"old":"/api/v1/financial/payments/student/:studentId","type":0,"val":"payments","end":""},{"old":"/api/v1/financial/payments/student/:studentId","type":0,"val":"student","end":""},{"old":"/api/v1/financial/payments/student/:studentId","type":1,"val":"studentId","end":""}],
    types: placeholder as Registry['financials.get_student_payments']['types'],
  },
  'financials.get_income_report': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/financial/reports/income',
    tokens: [{"old":"/api/v1/financial/reports/income","type":0,"val":"api","end":""},{"old":"/api/v1/financial/reports/income","type":0,"val":"v1","end":""},{"old":"/api/v1/financial/reports/income","type":0,"val":"financial","end":""},{"old":"/api/v1/financial/reports/income","type":0,"val":"reports","end":""},{"old":"/api/v1/financial/reports/income","type":0,"val":"income","end":""}],
    types: placeholder as Registry['financials.get_income_report']['types'],
  },
  'financials.get_financial_stats': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/financial/stats/financial',
    tokens: [{"old":"/api/v1/financial/stats/financial","type":0,"val":"api","end":""},{"old":"/api/v1/financial/stats/financial","type":0,"val":"v1","end":""},{"old":"/api/v1/financial/stats/financial","type":0,"val":"financial","end":""},{"old":"/api/v1/financial/stats/financial","type":0,"val":"stats","end":""},{"old":"/api/v1/financial/stats/financial","type":0,"val":"financial","end":""}],
    types: placeholder as Registry['financials.get_financial_stats']['types'],
  },
  'teachers.get_my_classes': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/teacher/my-classes',
    tokens: [{"old":"/api/v1/teacher/my-classes","type":0,"val":"api","end":""},{"old":"/api/v1/teacher/my-classes","type":0,"val":"v1","end":""},{"old":"/api/v1/teacher/my-classes","type":0,"val":"teacher","end":""},{"old":"/api/v1/teacher/my-classes","type":0,"val":"my-classes","end":""}],
    types: placeholder as Registry['teachers.get_my_classes']['types'],
  },
  'teachers.get_assignments': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/teacher/assignments',
    tokens: [{"old":"/api/v1/teacher/assignments","type":0,"val":"api","end":""},{"old":"/api/v1/teacher/assignments","type":0,"val":"v1","end":""},{"old":"/api/v1/teacher/assignments","type":0,"val":"teacher","end":""},{"old":"/api/v1/teacher/assignments","type":0,"val":"assignments","end":""}],
    types: placeholder as Registry['teachers.get_assignments']['types'],
  },
  'teachers.create_assignment': {
    methods: ["POST"],
    pattern: '/api/v1/teacher/assignments',
    tokens: [{"old":"/api/v1/teacher/assignments","type":0,"val":"api","end":""},{"old":"/api/v1/teacher/assignments","type":0,"val":"v1","end":""},{"old":"/api/v1/teacher/assignments","type":0,"val":"teacher","end":""},{"old":"/api/v1/teacher/assignments","type":0,"val":"assignments","end":""}],
    types: placeholder as Registry['teachers.create_assignment']['types'],
  },
  'teachers.grade_submission': {
    methods: ["POST"],
    pattern: '/api/v1/teacher/assignments/submissions/:id/grade',
    tokens: [{"old":"/api/v1/teacher/assignments/submissions/:id/grade","type":0,"val":"api","end":""},{"old":"/api/v1/teacher/assignments/submissions/:id/grade","type":0,"val":"v1","end":""},{"old":"/api/v1/teacher/assignments/submissions/:id/grade","type":0,"val":"teacher","end":""},{"old":"/api/v1/teacher/assignments/submissions/:id/grade","type":0,"val":"assignments","end":""},{"old":"/api/v1/teacher/assignments/submissions/:id/grade","type":0,"val":"submissions","end":""},{"old":"/api/v1/teacher/assignments/submissions/:id/grade","type":1,"val":"id","end":""},{"old":"/api/v1/teacher/assignments/submissions/:id/grade","type":0,"val":"grade","end":""}],
    types: placeholder as Registry['teachers.grade_submission']['types'],
  },
  'teachers.create_forum_topic': {
    methods: ["POST"],
    pattern: '/api/v1/teacher/forum/topics',
    tokens: [{"old":"/api/v1/teacher/forum/topics","type":0,"val":"api","end":""},{"old":"/api/v1/teacher/forum/topics","type":0,"val":"v1","end":""},{"old":"/api/v1/teacher/forum/topics","type":0,"val":"teacher","end":""},{"old":"/api/v1/teacher/forum/topics","type":0,"val":"forum","end":""},{"old":"/api/v1/teacher/forum/topics","type":0,"val":"topics","end":""}],
    types: placeholder as Registry['teachers.create_forum_topic']['types'],
  },
  'teachers.get_class_students_for_attendance': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/teacher/classes/:id/students',
    tokens: [{"old":"/api/v1/teacher/classes/:id/students","type":0,"val":"api","end":""},{"old":"/api/v1/teacher/classes/:id/students","type":0,"val":"v1","end":""},{"old":"/api/v1/teacher/classes/:id/students","type":0,"val":"teacher","end":""},{"old":"/api/v1/teacher/classes/:id/students","type":0,"val":"classes","end":""},{"old":"/api/v1/teacher/classes/:id/students","type":1,"val":"id","end":""},{"old":"/api/v1/teacher/classes/:id/students","type":0,"val":"students","end":""}],
    types: placeholder as Registry['teachers.get_class_students_for_attendance']['types'],
  },
  'teachers.get_class_attendance': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/teacher/attendance/class/:id',
    tokens: [{"old":"/api/v1/teacher/attendance/class/:id","type":0,"val":"api","end":""},{"old":"/api/v1/teacher/attendance/class/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/teacher/attendance/class/:id","type":0,"val":"teacher","end":""},{"old":"/api/v1/teacher/attendance/class/:id","type":0,"val":"attendance","end":""},{"old":"/api/v1/teacher/attendance/class/:id","type":0,"val":"class","end":""},{"old":"/api/v1/teacher/attendance/class/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['teachers.get_class_attendance']['types'],
  },
  'teachers.mark_attendance': {
    methods: ["POST"],
    pattern: '/api/v1/teacher/attendance',
    tokens: [{"old":"/api/v1/teacher/attendance","type":0,"val":"api","end":""},{"old":"/api/v1/teacher/attendance","type":0,"val":"v1","end":""},{"old":"/api/v1/teacher/attendance","type":0,"val":"teacher","end":""},{"old":"/api/v1/teacher/attendance","type":0,"val":"attendance","end":""}],
    types: placeholder as Registry['teachers.mark_attendance']['types'],
  },
  'parents.get_children': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/parent/children',
    tokens: [{"old":"/api/v1/parent/children","type":0,"val":"api","end":""},{"old":"/api/v1/parent/children","type":0,"val":"v1","end":""},{"old":"/api/v1/parent/children","type":0,"val":"parent","end":""},{"old":"/api/v1/parent/children","type":0,"val":"children","end":""}],
    types: placeholder as Registry['parents.get_children']['types'],
  },
  'parents.get_child_grades': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/parent/grades/child/:studentId',
    tokens: [{"old":"/api/v1/parent/grades/child/:studentId","type":0,"val":"api","end":""},{"old":"/api/v1/parent/grades/child/:studentId","type":0,"val":"v1","end":""},{"old":"/api/v1/parent/grades/child/:studentId","type":0,"val":"parent","end":""},{"old":"/api/v1/parent/grades/child/:studentId","type":0,"val":"grades","end":""},{"old":"/api/v1/parent/grades/child/:studentId","type":0,"val":"child","end":""},{"old":"/api/v1/parent/grades/child/:studentId","type":1,"val":"studentId","end":""}],
    types: placeholder as Registry['parents.get_child_grades']['types'],
  },
  'parents.send_message_to_teacher': {
    methods: ["POST"],
    pattern: '/api/v1/parent/messages/teacher',
    tokens: [{"old":"/api/v1/parent/messages/teacher","type":0,"val":"api","end":""},{"old":"/api/v1/parent/messages/teacher","type":0,"val":"v1","end":""},{"old":"/api/v1/parent/messages/teacher","type":0,"val":"parent","end":""},{"old":"/api/v1/parent/messages/teacher","type":0,"val":"messages","end":""},{"old":"/api/v1/parent/messages/teacher","type":0,"val":"teacher","end":""}],
    types: placeholder as Registry['parents.send_message_to_teacher']['types'],
  },
  'parents.get_child_payments': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/parent/payments/child/:studentId',
    tokens: [{"old":"/api/v1/parent/payments/child/:studentId","type":0,"val":"api","end":""},{"old":"/api/v1/parent/payments/child/:studentId","type":0,"val":"v1","end":""},{"old":"/api/v1/parent/payments/child/:studentId","type":0,"val":"parent","end":""},{"old":"/api/v1/parent/payments/child/:studentId","type":0,"val":"payments","end":""},{"old":"/api/v1/parent/payments/child/:studentId","type":0,"val":"child","end":""},{"old":"/api/v1/parent/payments/child/:studentId","type":1,"val":"studentId","end":""}],
    types: placeholder as Registry['parents.get_child_payments']['types'],
  },
  'parents.justify_absence': {
    methods: ["POST"],
    pattern: '/api/v1/parent/absence/justify',
    tokens: [{"old":"/api/v1/parent/absence/justify","type":0,"val":"api","end":""},{"old":"/api/v1/parent/absence/justify","type":0,"val":"v1","end":""},{"old":"/api/v1/parent/absence/justify","type":0,"val":"parent","end":""},{"old":"/api/v1/parent/absence/justify","type":0,"val":"absence","end":""},{"old":"/api/v1/parent/absence/justify","type":0,"val":"justify","end":""}],
    types: placeholder as Registry['parents.justify_absence']['types'],
  },
  'students.get_my_profile': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/student/my-profile',
    tokens: [{"old":"/api/v1/student/my-profile","type":0,"val":"api","end":""},{"old":"/api/v1/student/my-profile","type":0,"val":"v1","end":""},{"old":"/api/v1/student/my-profile","type":0,"val":"student","end":""},{"old":"/api/v1/student/my-profile","type":0,"val":"my-profile","end":""}],
    types: placeholder as Registry['students.get_my_profile']['types'],
  },
  'students.get_my_grades': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/student/my-grades',
    tokens: [{"old":"/api/v1/student/my-grades","type":0,"val":"api","end":""},{"old":"/api/v1/student/my-grades","type":0,"val":"v1","end":""},{"old":"/api/v1/student/my-grades","type":0,"val":"student","end":""},{"old":"/api/v1/student/my-grades","type":0,"val":"my-grades","end":""}],
    types: placeholder as Registry['students.get_my_grades']['types'],
  },
  'students.get_my_report_card': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/student/my-report-card',
    tokens: [{"old":"/api/v1/student/my-report-card","type":0,"val":"api","end":""},{"old":"/api/v1/student/my-report-card","type":0,"val":"v1","end":""},{"old":"/api/v1/student/my-report-card","type":0,"val":"student","end":""},{"old":"/api/v1/student/my-report-card","type":0,"val":"my-report-card","end":""}],
    types: placeholder as Registry['students.get_my_report_card']['types'],
  },
  'students.get_my_discipline': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/student/my-discipline',
    tokens: [{"old":"/api/v1/student/my-discipline","type":0,"val":"api","end":""},{"old":"/api/v1/student/my-discipline","type":0,"val":"v1","end":""},{"old":"/api/v1/student/my-discipline","type":0,"val":"student","end":""},{"old":"/api/v1/student/my-discipline","type":0,"val":"my-discipline","end":""}],
    types: placeholder as Registry['students.get_my_discipline']['types'],
  },
  'students.get_assignments': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/student/assignments',
    tokens: [{"old":"/api/v1/student/assignments","type":0,"val":"api","end":""},{"old":"/api/v1/student/assignments","type":0,"val":"v1","end":""},{"old":"/api/v1/student/assignments","type":0,"val":"student","end":""},{"old":"/api/v1/student/assignments","type":0,"val":"assignments","end":""}],
    types: placeholder as Registry['students.get_assignments']['types'],
  },
  'students.submit_assignment': {
    methods: ["POST"],
    pattern: '/api/v1/student/assignments/submit',
    tokens: [{"old":"/api/v1/student/assignments/submit","type":0,"val":"api","end":""},{"old":"/api/v1/student/assignments/submit","type":0,"val":"v1","end":""},{"old":"/api/v1/student/assignments/submit","type":0,"val":"student","end":""},{"old":"/api/v1/student/assignments/submit","type":0,"val":"assignments","end":""},{"old":"/api/v1/student/assignments/submit","type":0,"val":"submit","end":""}],
    types: placeholder as Registry['students.submit_assignment']['types'],
  },
  'students.get_my_forum_questions': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/student/forum/questions',
    tokens: [{"old":"/api/v1/student/forum/questions","type":0,"val":"api","end":""},{"old":"/api/v1/student/forum/questions","type":0,"val":"v1","end":""},{"old":"/api/v1/student/forum/questions","type":0,"val":"student","end":""},{"old":"/api/v1/student/forum/questions","type":0,"val":"forum","end":""},{"old":"/api/v1/student/forum/questions","type":0,"val":"questions","end":""}],
    types: placeholder as Registry['students.get_my_forum_questions']['types'],
  },
  'students.post_forum_question': {
    methods: ["POST"],
    pattern: '/api/v1/student/forum/questions',
    tokens: [{"old":"/api/v1/student/forum/questions","type":0,"val":"api","end":""},{"old":"/api/v1/student/forum/questions","type":0,"val":"v1","end":""},{"old":"/api/v1/student/forum/questions","type":0,"val":"student","end":""},{"old":"/api/v1/student/forum/questions","type":0,"val":"forum","end":""},{"old":"/api/v1/student/forum/questions","type":0,"val":"questions","end":""}],
    types: placeholder as Registry['students.post_forum_question']['types'],
  },
  'students.send_message_to_teacher': {
    methods: ["POST"],
    pattern: '/api/v1/student/messages/teacher',
    tokens: [{"old":"/api/v1/student/messages/teacher","type":0,"val":"api","end":""},{"old":"/api/v1/student/messages/teacher","type":0,"val":"v1","end":""},{"old":"/api/v1/student/messages/teacher","type":0,"val":"student","end":""},{"old":"/api/v1/student/messages/teacher","type":0,"val":"messages","end":""},{"old":"/api/v1/student/messages/teacher","type":0,"val":"teacher","end":""}],
    types: placeholder as Registry['students.send_message_to_teacher']['types'],
  },
  'students.get_my_timetable': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/student/timetable',
    tokens: [{"old":"/api/v1/student/timetable","type":0,"val":"api","end":""},{"old":"/api/v1/student/timetable","type":0,"val":"v1","end":""},{"old":"/api/v1/student/timetable","type":0,"val":"student","end":""},{"old":"/api/v1/student/timetable","type":0,"val":"timetable","end":""}],
    types: placeholder as Registry['students.get_my_timetable']['types'],
  },
  'students.get_my_attendance': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/student/attendance',
    tokens: [{"old":"/api/v1/student/attendance","type":0,"val":"api","end":""},{"old":"/api/v1/student/attendance","type":0,"val":"v1","end":""},{"old":"/api/v1/student/attendance","type":0,"val":"student","end":""},{"old":"/api/v1/student/attendance","type":0,"val":"attendance","end":""}],
    types: placeholder as Registry['students.get_my_attendance']['types'],
  },
  'students.request_transfer': {
    methods: ["POST"],
    pattern: '/api/v1/student/transfers/request',
    tokens: [{"old":"/api/v1/student/transfers/request","type":0,"val":"api","end":""},{"old":"/api/v1/student/transfers/request","type":0,"val":"v1","end":""},{"old":"/api/v1/student/transfers/request","type":0,"val":"student","end":""},{"old":"/api/v1/student/transfers/request","type":0,"val":"transfers","end":""},{"old":"/api/v1/student/transfers/request","type":0,"val":"request","end":""}],
    types: placeholder as Registry['students.request_transfer']['types'],
  },
  'disciplines.get_students': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/discipline/students',
    tokens: [{"old":"/api/v1/discipline/students","type":0,"val":"api","end":""},{"old":"/api/v1/discipline/students","type":0,"val":"v1","end":""},{"old":"/api/v1/discipline/students","type":0,"val":"discipline","end":""},{"old":"/api/v1/discipline/students","type":0,"val":"students","end":""}],
    types: placeholder as Registry['disciplines.get_students']['types'],
  },
  'disciplines.get_student_details': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/discipline/students/:id',
    tokens: [{"old":"/api/v1/discipline/students/:id","type":0,"val":"api","end":""},{"old":"/api/v1/discipline/students/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/discipline/students/:id","type":0,"val":"discipline","end":""},{"old":"/api/v1/discipline/students/:id","type":0,"val":"students","end":""},{"old":"/api/v1/discipline/students/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['disciplines.get_student_details']['types'],
  },
  'disciplines.report_incident': {
    methods: ["POST"],
    pattern: '/api/v1/discipline/incidents',
    tokens: [{"old":"/api/v1/discipline/incidents","type":0,"val":"api","end":""},{"old":"/api/v1/discipline/incidents","type":0,"val":"v1","end":""},{"old":"/api/v1/discipline/incidents","type":0,"val":"discipline","end":""},{"old":"/api/v1/discipline/incidents","type":0,"val":"incidents","end":""}],
    types: placeholder as Registry['disciplines.report_incident']['types'],
  },
  'disciplines.delete_incident': {
    methods: ["DELETE"],
    pattern: '/api/v1/discipline/incidents/:id',
    tokens: [{"old":"/api/v1/discipline/incidents/:id","type":0,"val":"api","end":""},{"old":"/api/v1/discipline/incidents/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/discipline/incidents/:id","type":0,"val":"discipline","end":""},{"old":"/api/v1/discipline/incidents/:id","type":0,"val":"incidents","end":""},{"old":"/api/v1/discipline/incidents/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['disciplines.delete_incident']['types'],
  },
  'disciplines.apply_sanction': {
    methods: ["POST"],
    pattern: '/api/v1/discipline/sanctions',
    tokens: [{"old":"/api/v1/discipline/sanctions","type":0,"val":"api","end":""},{"old":"/api/v1/discipline/sanctions","type":0,"val":"v1","end":""},{"old":"/api/v1/discipline/sanctions","type":0,"val":"discipline","end":""},{"old":"/api/v1/discipline/sanctions","type":0,"val":"sanctions","end":""}],
    types: placeholder as Registry['disciplines.apply_sanction']['types'],
  },
  'disciplines.notify_parent': {
    methods: ["POST"],
    pattern: '/api/v1/discipline/notify-parent',
    tokens: [{"old":"/api/v1/discipline/notify-parent","type":0,"val":"api","end":""},{"old":"/api/v1/discipline/notify-parent","type":0,"val":"v1","end":""},{"old":"/api/v1/discipline/notify-parent","type":0,"val":"discipline","end":""},{"old":"/api/v1/discipline/notify-parent","type":0,"val":"notify-parent","end":""}],
    types: placeholder as Registry['disciplines.notify_parent']['types'],
  },
  'messages.get_messages': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/communication/messages',
    tokens: [{"old":"/api/v1/communication/messages","type":0,"val":"api","end":""},{"old":"/api/v1/communication/messages","type":0,"val":"v1","end":""},{"old":"/api/v1/communication/messages","type":0,"val":"communication","end":""},{"old":"/api/v1/communication/messages","type":0,"val":"messages","end":""}],
    types: placeholder as Registry['messages.get_messages']['types'],
  },
  'messages.send_message': {
    methods: ["POST"],
    pattern: '/api/v1/communication/messages/send',
    tokens: [{"old":"/api/v1/communication/messages/send","type":0,"val":"api","end":""},{"old":"/api/v1/communication/messages/send","type":0,"val":"v1","end":""},{"old":"/api/v1/communication/messages/send","type":0,"val":"communication","end":""},{"old":"/api/v1/communication/messages/send","type":0,"val":"messages","end":""},{"old":"/api/v1/communication/messages/send","type":0,"val":"send","end":""}],
    types: placeholder as Registry['messages.send_message']['types'],
  },
  'messages.mark_as_read': {
    methods: ["PUT"],
    pattern: '/api/v1/communication/messages/read',
    tokens: [{"old":"/api/v1/communication/messages/read","type":0,"val":"api","end":""},{"old":"/api/v1/communication/messages/read","type":0,"val":"v1","end":""},{"old":"/api/v1/communication/messages/read","type":0,"val":"communication","end":""},{"old":"/api/v1/communication/messages/read","type":0,"val":"messages","end":""},{"old":"/api/v1/communication/messages/read","type":0,"val":"read","end":""}],
    types: placeholder as Registry['messages.mark_as_read']['types'],
  },
  'messages.get_conversations': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/communication/conversations',
    tokens: [{"old":"/api/v1/communication/conversations","type":0,"val":"api","end":""},{"old":"/api/v1/communication/conversations","type":0,"val":"v1","end":""},{"old":"/api/v1/communication/conversations","type":0,"val":"communication","end":""},{"old":"/api/v1/communication/conversations","type":0,"val":"conversations","end":""}],
    types: placeholder as Registry['messages.get_conversations']['types'],
  },
  'messages.get_conversation': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/communication/conversations/:userId',
    tokens: [{"old":"/api/v1/communication/conversations/:userId","type":0,"val":"api","end":""},{"old":"/api/v1/communication/conversations/:userId","type":0,"val":"v1","end":""},{"old":"/api/v1/communication/conversations/:userId","type":0,"val":"communication","end":""},{"old":"/api/v1/communication/conversations/:userId","type":0,"val":"conversations","end":""},{"old":"/api/v1/communication/conversations/:userId","type":1,"val":"userId","end":""}],
    types: placeholder as Registry['messages.get_conversation']['types'],
  },
  'communication.send_global_communication': {
    methods: ["POST"],
    pattern: '/api/v1/communication/communications/global',
    tokens: [{"old":"/api/v1/communication/communications/global","type":0,"val":"api","end":""},{"old":"/api/v1/communication/communications/global","type":0,"val":"v1","end":""},{"old":"/api/v1/communication/communications/global","type":0,"val":"communication","end":""},{"old":"/api/v1/communication/communications/global","type":0,"val":"communications","end":""},{"old":"/api/v1/communication/communications/global","type":0,"val":"global","end":""}],
    types: placeholder as Registry['communication.send_global_communication']['types'],
  },
  'communication.send_school_communication': {
    methods: ["POST"],
    pattern: '/api/v1/communication/communications/school',
    tokens: [{"old":"/api/v1/communication/communications/school","type":0,"val":"api","end":""},{"old":"/api/v1/communication/communications/school","type":0,"val":"v1","end":""},{"old":"/api/v1/communication/communications/school","type":0,"val":"communication","end":""},{"old":"/api/v1/communication/communications/school","type":0,"val":"communications","end":""},{"old":"/api/v1/communication/communications/school","type":0,"val":"school","end":""}],
    types: placeholder as Registry['communication.send_school_communication']['types'],
  },
  'inter_schools.search_schools': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/inter-school/search',
    tokens: [{"old":"/api/v1/inter-school/search","type":0,"val":"api","end":""},{"old":"/api/v1/inter-school/search","type":0,"val":"v1","end":""},{"old":"/api/v1/inter-school/search","type":0,"val":"inter-school","end":""},{"old":"/api/v1/inter-school/search","type":0,"val":"search","end":""}],
    types: placeholder as Registry['inter_schools.search_schools']['types'],
  },
  'inter_schools.get_school_public_info': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/inter-school/:id/info',
    tokens: [{"old":"/api/v1/inter-school/:id/info","type":0,"val":"api","end":""},{"old":"/api/v1/inter-school/:id/info","type":0,"val":"v1","end":""},{"old":"/api/v1/inter-school/:id/info","type":0,"val":"inter-school","end":""},{"old":"/api/v1/inter-school/:id/info","type":1,"val":"id","end":""},{"old":"/api/v1/inter-school/:id/info","type":0,"val":"info","end":""}],
    types: placeholder as Registry['inter_schools.get_school_public_info']['types'],
  },
  'inter_schools.start_exchange': {
    methods: ["POST"],
    pattern: '/api/v1/inter-school/exchanges',
    tokens: [{"old":"/api/v1/inter-school/exchanges","type":0,"val":"api","end":""},{"old":"/api/v1/inter-school/exchanges","type":0,"val":"v1","end":""},{"old":"/api/v1/inter-school/exchanges","type":0,"val":"inter-school","end":""},{"old":"/api/v1/inter-school/exchanges","type":0,"val":"exchanges","end":""}],
    types: placeholder as Registry['inter_schools.start_exchange']['types'],
  },
  'inter_schools.get_best_practices': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/inter-school/best-practices',
    tokens: [{"old":"/api/v1/inter-school/best-practices","type":0,"val":"api","end":""},{"old":"/api/v1/inter-school/best-practices","type":0,"val":"v1","end":""},{"old":"/api/v1/inter-school/best-practices","type":0,"val":"inter-school","end":""},{"old":"/api/v1/inter-school/best-practices","type":0,"val":"best-practices","end":""}],
    types: placeholder as Registry['inter_schools.get_best_practices']['types'],
  },
  'inter_schools.share_best_practice': {
    methods: ["POST"],
    pattern: '/api/v1/inter-school/best-practices/share',
    tokens: [{"old":"/api/v1/inter-school/best-practices/share","type":0,"val":"api","end":""},{"old":"/api/v1/inter-school/best-practices/share","type":0,"val":"v1","end":""},{"old":"/api/v1/inter-school/best-practices/share","type":0,"val":"inter-school","end":""},{"old":"/api/v1/inter-school/best-practices/share","type":0,"val":"best-practices","end":""},{"old":"/api/v1/inter-school/best-practices/share","type":0,"val":"share","end":""}],
    types: placeholder as Registry['inter_schools.share_best_practice']['types'],
  },
  'inter_schools.get_events': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/inter-school/events',
    tokens: [{"old":"/api/v1/inter-school/events","type":0,"val":"api","end":""},{"old":"/api/v1/inter-school/events","type":0,"val":"v1","end":""},{"old":"/api/v1/inter-school/events","type":0,"val":"inter-school","end":""},{"old":"/api/v1/inter-school/events","type":0,"val":"events","end":""}],
    types: placeholder as Registry['inter_schools.get_events']['types'],
  },
  'inter_schools.create_event': {
    methods: ["POST"],
    pattern: '/api/v1/inter-school/events',
    tokens: [{"old":"/api/v1/inter-school/events","type":0,"val":"api","end":""},{"old":"/api/v1/inter-school/events","type":0,"val":"v1","end":""},{"old":"/api/v1/inter-school/events","type":0,"val":"inter-school","end":""},{"old":"/api/v1/inter-school/events","type":0,"val":"events","end":""}],
    types: placeholder as Registry['inter_schools.create_event']['types'],
  },
  'inter_schools.join_event': {
    methods: ["POST"],
    pattern: '/api/v1/inter-school/events/join',
    tokens: [{"old":"/api/v1/inter-school/events/join","type":0,"val":"api","end":""},{"old":"/api/v1/inter-school/events/join","type":0,"val":"v1","end":""},{"old":"/api/v1/inter-school/events/join","type":0,"val":"inter-school","end":""},{"old":"/api/v1/inter-school/events/join","type":0,"val":"events","end":""},{"old":"/api/v1/inter-school/events/join","type":0,"val":"join","end":""}],
    types: placeholder as Registry['inter_schools.join_event']['types'],
  },
  'admin.get_users': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/admin/users',
    tokens: [{"old":"/api/v1/admin/users","type":0,"val":"api","end":""},{"old":"/api/v1/admin/users","type":0,"val":"v1","end":""},{"old":"/api/v1/admin/users","type":0,"val":"admin","end":""},{"old":"/api/v1/admin/users","type":0,"val":"users","end":""}],
    types: placeholder as Registry['admin.get_users']['types'],
  },
  'admin.create_user': {
    methods: ["POST"],
    pattern: '/api/v1/admin/users',
    tokens: [{"old":"/api/v1/admin/users","type":0,"val":"api","end":""},{"old":"/api/v1/admin/users","type":0,"val":"v1","end":""},{"old":"/api/v1/admin/users","type":0,"val":"admin","end":""},{"old":"/api/v1/admin/users","type":0,"val":"users","end":""}],
    types: placeholder as Registry['admin.create_user']['types'],
  },
  'admin.update_user': {
    methods: ["PUT"],
    pattern: '/api/v1/admin/users/:id',
    tokens: [{"old":"/api/v1/admin/users/:id","type":0,"val":"api","end":""},{"old":"/api/v1/admin/users/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/admin/users/:id","type":0,"val":"admin","end":""},{"old":"/api/v1/admin/users/:id","type":0,"val":"users","end":""},{"old":"/api/v1/admin/users/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['admin.update_user']['types'],
  },
  'admin.delete_user': {
    methods: ["DELETE"],
    pattern: '/api/v1/admin/users/:id',
    tokens: [{"old":"/api/v1/admin/users/:id","type":0,"val":"api","end":""},{"old":"/api/v1/admin/users/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/admin/users/:id","type":0,"val":"admin","end":""},{"old":"/api/v1/admin/users/:id","type":0,"val":"users","end":""},{"old":"/api/v1/admin/users/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['admin.delete_user']['types'],
  },
  'admin.activate_user': {
    methods: ["POST"],
    pattern: '/api/v1/admin/users/:id/activate',
    tokens: [{"old":"/api/v1/admin/users/:id/activate","type":0,"val":"api","end":""},{"old":"/api/v1/admin/users/:id/activate","type":0,"val":"v1","end":""},{"old":"/api/v1/admin/users/:id/activate","type":0,"val":"admin","end":""},{"old":"/api/v1/admin/users/:id/activate","type":0,"val":"users","end":""},{"old":"/api/v1/admin/users/:id/activate","type":1,"val":"id","end":""},{"old":"/api/v1/admin/users/:id/activate","type":0,"val":"activate","end":""}],
    types: placeholder as Registry['admin.activate_user']['types'],
  },
  'admin.suspend_user': {
    methods: ["POST"],
    pattern: '/api/v1/admin/users/:id/suspend',
    tokens: [{"old":"/api/v1/admin/users/:id/suspend","type":0,"val":"api","end":""},{"old":"/api/v1/admin/users/:id/suspend","type":0,"val":"v1","end":""},{"old":"/api/v1/admin/users/:id/suspend","type":0,"val":"admin","end":""},{"old":"/api/v1/admin/users/:id/suspend","type":0,"val":"users","end":""},{"old":"/api/v1/admin/users/:id/suspend","type":1,"val":"id","end":""},{"old":"/api/v1/admin/users/:id/suspend","type":0,"val":"suspend","end":""}],
    types: placeholder as Registry['admin.suspend_user']['types'],
  },
  'admin.get_roles': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/admin/roles',
    tokens: [{"old":"/api/v1/admin/roles","type":0,"val":"api","end":""},{"old":"/api/v1/admin/roles","type":0,"val":"v1","end":""},{"old":"/api/v1/admin/roles","type":0,"val":"admin","end":""},{"old":"/api/v1/admin/roles","type":0,"val":"roles","end":""}],
    types: placeholder as Registry['admin.get_roles']['types'],
  },
  'admin.create_role': {
    methods: ["POST"],
    pattern: '/api/v1/admin/roles',
    tokens: [{"old":"/api/v1/admin/roles","type":0,"val":"api","end":""},{"old":"/api/v1/admin/roles","type":0,"val":"v1","end":""},{"old":"/api/v1/admin/roles","type":0,"val":"admin","end":""},{"old":"/api/v1/admin/roles","type":0,"val":"roles","end":""}],
    types: placeholder as Registry['admin.create_role']['types'],
  },
  'admin.update_role': {
    methods: ["PUT"],
    pattern: '/api/v1/admin/roles/:id',
    tokens: [{"old":"/api/v1/admin/roles/:id","type":0,"val":"api","end":""},{"old":"/api/v1/admin/roles/:id","type":0,"val":"v1","end":""},{"old":"/api/v1/admin/roles/:id","type":0,"val":"admin","end":""},{"old":"/api/v1/admin/roles/:id","type":0,"val":"roles","end":""},{"old":"/api/v1/admin/roles/:id","type":1,"val":"id","end":""}],
    types: placeholder as Registry['admin.update_role']['types'],
  },
  'admin.get_system_logs': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/admin/logs',
    tokens: [{"old":"/api/v1/admin/logs","type":0,"val":"api","end":""},{"old":"/api/v1/admin/logs","type":0,"val":"v1","end":""},{"old":"/api/v1/admin/logs","type":0,"val":"admin","end":""},{"old":"/api/v1/admin/logs","type":0,"val":"logs","end":""}],
    types: placeholder as Registry['admin.get_system_logs']['types'],
  },
  'admin.get_user_activity_logs': {
    methods: ["GET","HEAD"],
    pattern: '/api/v1/admin/logs/users',
    tokens: [{"old":"/api/v1/admin/logs/users","type":0,"val":"api","end":""},{"old":"/api/v1/admin/logs/users","type":0,"val":"v1","end":""},{"old":"/api/v1/admin/logs/users","type":0,"val":"admin","end":""},{"old":"/api/v1/admin/logs/users","type":0,"val":"logs","end":""},{"old":"/api/v1/admin/logs/users","type":0,"val":"users","end":""}],
    types: placeholder as Registry['admin.get_user_activity_logs']['types'],
  },
} as const satisfies Record<string, AdonisEndpoint>

export { routes }

export const registry = {
  routes,
  $tree: {} as ApiDefinition,
}

declare module '@tuyau/core/types' {
  export interface UserRegistry {
    routes: typeof routes
    $tree: ApiDefinition
  }
}
